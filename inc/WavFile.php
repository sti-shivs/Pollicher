...
class Shivs_Poll_WavFile
{
    ...
    /** @var int The audio format - Shivs_Poll_WavFile::WAVE_FORMAT_* */
    protected $_audioFormat;

    /** @var int The audio subformat - Shivs_Poll_WavFile::WAVE_SUBFORMAT_* */
    ...
    /**
     * Shivs_Poll_WavFile Constructor.
     *
     * <code>
     * $wav1 = new Shivs_Poll_WavFile(2, 44100, 16);         // new wav with 2 channels, at 44100 samples/sec and 16 bits per sample
     * $wav2 = new Shivs_Poll_WavFile('./audio/sound.wav');  // open and read wav file
     * </code>
     *
     ...
     * @throws Shivs_Poll_WavFormatException
     * @throws Shivs_Poll_WavFileException
     */
    ...
    public function setNumChannels($numChannels) {
        if ($numChannels < 1 || $numChannels > self::MAX_CHANNEL) {
            throw new Shivs_Poll_WavFileException('Unsupported number of channels. Only up to ' . self::MAX_CHANNEL . ' channels are supported.');
    ...
    public function setChannelMask($channelMask = self::SPEAKER_DEFAULT) {
        if ($channelMask != 0) {
            ...
            if ($n != $this->_numChannels || (((int)$channelMask | self::SPEAKER_ALL) != self::SPEAKER_ALL)) {
                throw new Shivs_Poll_WavFileException('Invalid channel mask. The number of channels does not match the number of locations in the mask.');
            }
        }
    ...
    public function setSampleRate($sampleRate) {
        if ($sampleRate < 1 || $sampleRate > self::MAX_SAMPLERATE) {
            throw new Shivs_Poll_WavFileException('Invalid sample rate.');
        } elseif ($this->_samples !== '') {
    ...
    public function setBitsPerSample($bitsPerSample) {
        if (!in_array($bitsPerSample, array(8, 16, 24, 32))) {
            throw new Shivs_Poll_WavFileException('Unsupported bits per sample. Only 8, 16, 24 and 32 bits are supported.');
        } elseif ($this->_samples !== '') {
    ...
    protected function setValidBitsPerSample($validBitsPerSample = null) {
        if (is_null($validBitsPerSample)) {
            ...
            if ($validBitsPerSample < 1 || $validBitsPerSample > $this->_bitsPerSample) {
                throw new Shivs_Poll_WavFileException('ValidBitsPerSample cannot be greater than BitsPerSample.');
            }
    ...
    public function setSamples(&$samples = '') {
        if (strlen($samples) % $this->_blockAlign != 0) {
            throw new Shivs_Poll_WavFileException('Incorrect samples size. Has to be a multiple of BlockAlign.');
        }
    ...
    public function save($filename)
    {
        ...
        if (!is_resource($fp)) {
            throw new Shivs_Poll_WavFileException('Failed to open "' . $filename . '" for writing.');
        }
    ...
    public function openWav($filename, $readData = true)
    {
        ...
        if (!file_exists($filename)) {
            throw new Shivs_Poll_WavFileException('Failed to open "' . $filename . '". File not found.');
        } elseif (!is_readable($filename)) {
            throw new Shivs_Poll_WavFileException('Failed to open "' . $filename . '". File is not readable.');
        ...
        if (!is_resource($this->_fp)) {
            throw new Shivs_Poll_WavFileException('Failed to open "' . $filename . '".');
        }
    ...
    public function setWavData(&$data, $free = true)
    {
        ...
        if (!is_resource($this->_fp)) {
            throw new Shivs_Poll_WavFileException('Failed to open memory stream to write wav data. Use openWav() instead.');
        }
    ...
    {
        if (!is_resource($this->_fp)) {
            throw new Shivs_Poll_WavFileException('No wav file open. Use openWav() first.');
        }

        try {
            $this->readWavHeader();
        } catch (Shivs_Poll_WavFileException $ex) {
            $this->closeWav();
            throw $ex;
        }
    ...
    protected function readWavHeader()
    {
        ...
        if (strlen($header) < 36) {
            throw new Shivs_Poll_WavFormatException('Not wav format. Header too short.', 1);
        }
        ...
        if ($RIFF['ChunkID'] != 0x52494646) {  // "RIFF"
            throw new Shivs_Poll_WavFormatException('Not wav format. "RIFF" signature missing.', 2);
        }
        ...
        if ($actualSize - 8 < $RIFF['ChunkSize']) {
            ...
            //throw new Shivs_Poll_WavFormatException('"RIFF" chunk size does not match actual file size. Found ' . $RIFF['ChunkSize'] . ', expected ' . ($actualSize - 8) . '.', 3);
        }

        if ($RIFF['Format'] != 0x57415645) {  // "WAVE"
            throw new Shivs_Poll_WavFormatException('Not wav format. "RIFF" chunk format is not "WAVE".', 4);
        }
        ...
        if ($fmt['SubchunkID'] != 0x666d7420) {  // "fmt "
            throw new Shivs_Poll_WavFormatException('Bad wav header. Expected "fmt " subchunk.', 11);
        }

        if ($fmt['SubchunkSize'] < 16) {
            throw new Shivs_Poll_WavFormatException('Bad "fmt " subchunk size.', 12);
        }
        ...
        {
            throw new Shivs_Poll_WavFormatException('Unsupported audio format. Only PCM or IEEE FLOAT (EXTENSIBLE) audio is supported.', 13);
        }

        if ($fmt['NumChannels'] < 1 || $fmt['NumChannels'] > self::MAX_CHANNEL) {
            throw new Shivs_Poll_WavFormatException('Invalid number of channels in "fmt " subchunk.', 14);
        }

        if ($fmt['SampleRate'] < 1 || $fmt['SampleRate'] > self::MAX_SAMPLERATE) {
            throw new Shivs_Poll_WavFormatException('Invalid sample rate in "fmt " subchunk.', 15);
        }
        ...
        {
            throw new Shivs_Poll_WavFormatException('Only 8, 16 and 24-bit PCM and 32-bit IEEE FLOAT (EXTENSIBLE) audio is supported.', 16);
        }
        ...
        if ($blockAlign != $fmt['BlockAlign']) {
            ...
            //throw new Shivs_Poll_WavFormatException('Invalid block align in "fmt " subchunk. Found ' . $fmt['BlockAlign'] . ', expected ' . $blockAlign . '.', 17);
        }
        ...
        if ($byteRate != $fmt['ByteRate']) {
            ...
            //throw new Shivs_Poll_WavFormatException('Invalid average byte rate in "fmt " subchunk. Found ' . $fmt['ByteRate'] . ', expected ' . $byteRate . '.', 18);
        }
        ...
        if ($fmt['AudioFormat'] == self::WAVE_FORMAT_EXTENSIBLE) {
            if (strlen($extendedFmt) < 24) {
                throw new Shivs_Poll_WavFormatException('Invalid EXTENSIBLE "fmt " subchunk size. Found ' . $fmt['SubchunkSize'] . ', expected at least 40.', 19);
            }
            ...
            if ($extensibleFmt['Size'] != 22) {
                ...
                //throw new Shivs_Poll_WavFormatException('Invaid extension size in EXTENSIBLE "fmt " subchunk.', 20);
            }
            ...
            if ($extensibleFmt['ValidBitsPerSample'] != $fmt['BitsPerSample']) {
                ...
                //throw new Shivs_Poll_WavFormatException('Invaid or unsupported valid bits per sample in EXTENSIBLE "fmt " subchunk.', 21);
            }
            ...
            if ($extensibleFmt['ChannelMask'] != 0) {
                ...
                if ($n != $fmt['NumChannels'] || (((int)$extensibleFmt['ChannelMask'] | self::SPEAKER_ALL) != self::SPEAKER_ALL)) {
                    ...
                    //throw new Shivs_Poll_WavFormatException('Invalid channel mask in EXTENSIBLE "fmt " subchunk. The number of channels does not match the number of locations in the mask.', 22);
                }
            }
        ...
        while (!feof($this->_fp)) {
            ...
            if (strlen($subchunkHeader) < 8) {
                throw new Shivs_Poll_WavFormatException('Missing "data" subchunk.', 101);
            }
            ...
            if ($subchunk['SubchunkID'] == 0x66616374) {        // "fact"
                ...
                if (strlen($subchunkData) < 4) {
                    throw new Shivs_Poll_WavFormatException('Invalid "fact" subchunk.', 102);
                }
                ...
                throw new Shivs_Poll_WavFormatException('Wave List Chunk ("wavl" subchunk) is not supported.', 106);
            } else {
                ...
                if ( $subchunk['SubchunkSize'] < 0
                  || fseek($this->_fp, $subchunk['SubchunkSize'] + ($subchunk['SubchunkSize'] & 1), SEEK_CUR) !== 0) {  // also skip padding byte
                    throw new Shivs_Poll_WavFormatException('Invalid subchunk (0x' . dechex($subchunk['SubchunkID']) . ') encountered.', 103);
                }
            }
        }
        ...
        if ($dataSubchunk['SubchunkSize'] < 0 || $actualSize - $dataOffset < $dataSubchunk['SubchunkSize']) {
            ...
            //throw new Shivs_Poll_WavFormatException('Invalid "data" subchunk size.', 104);
        }
        ...
        if ($factSubchunk['SampleLength'] != $numBlocks) {
            ...
            //throw new Shivs_Poll_WavFormatException('Invalid sample length in "fact" subchunk.', 105);
        }
    ...
    public function readWavData($dataOffset = 0, $dataSize = null)
    {
        ...
        if ($dataOffset < 0 || $dataOffset % $this->getBlockAlign() > 0) {
            throw new Shivs_Poll_WavFileException('Invalid data offset. Has to be a multiple of BlockAlign.');
        }
        ...
        if (is_null($dataSize)) {
            ...
            throw new Shivs_Poll_WavFileException('Invalid data size to read. Has to be a multiple of BlockAlign.');
        }


        // skip offset
        if ($dataOffset > 0 && fseek($this->_fp, $dataOffset, SEEK_CUR) !== 0) {
            throw new Shivs_Poll_WavFileException('Seeking to data offset failed.');
        }
    ...
    public function setSampleBlock($sampleBlock, $blockNum)
    {
        ...
        if (!isset($sampleBlock[$blockAlign - 1]) || isset($sampleBlock[$blockAlign])) {  // faster than: if (strlen($sampleBlock) != $blockAlign)
            throw new Shivs_Poll_WavFileException('Incorrect sample block size. Got ' . strlen($sampleBlock) . ', expected ' . $blockAlign . '.');
        }
        ...
        if ($blockNum > $numBlocks || $blockNum < 0) {  // allow appending
            throw new Shivs_Poll_WavFileException('Sample block number is out of range.');
        }
    ...
    public function getSampleValue($blockNum, $channelNum)
    {
        ...
        if ($channelNum < 1 || $channelNum > $this->_numChannels) {
            throw new Shivs_Poll_WavFileException('Channel number is out of range.');
        }
    ...
    public function setSampleValue($sampleFloat, $blockNum, $channelNum)
    {
        ...
        if (($offset + $sampleBytes > $dataSize && $offset != $dataSize) || $offset < 0) { // allow appending
            throw new Shivs_Poll_WavFileException('Sample block or channel number is out of range.');
        }
     ...
     * $wav->filter(
     *      array(
     *          Shivs_Poll_WavFile::FILTER_MIX => array(          // Filter for mixing 2 Shivs_Poll_WavFile instances.
     *              'wav' => $wav2,                    // (Required) The Shivs_Poll_WavFile to mix into this WhavFile. If no optional arguments are given, can be passed without the array.
     ...
     *          ),
     *          Shivs_Poll_WavFile::FILTER_NORMALIZE => 0.6,      // (Required) Normalization of (mixed) audio samples - see threshold parameter for normalizeSample().
     *          Shivs_Poll_WavFile::FILTER_DEGRADE => 0.9         // (Required) Introduce random noise. The quality relative to the amplitude. 1 = no noise, 0 = max. noise.
     *      ),
     *      0,                                         // (Optional) The block number of this Shivs_Poll_WavFile to start with.
    ...
    public function filter($filters, $blockOffset = 0, $numBlocks = null)
    {
        ...
        if (array_key_exists(self::FILTER_MIX, $filters)) {
            ...
            if (!($mix_wav instanceof Shivs_Poll_WavFile)) {
                throw new Shivs_Poll_WavFileException("WavFile to mix is missing or invalid.");
            } elseif ($mix_wav->getSampleRate() != $this->getSampleRate()) {
                throw new Shivs_Poll_WavFileException("Sample rate of Shivs_Poll_WavFile to mix does not match.");
            } else if ($mix_wav->getNumChannels() != $this->getNumChannels()) {
                throw new Shivs_Poll_WavFileException("Number of channels of Shivs_Poll_WavFile to mix does not match.");
            }
     ...
     *
     * @param Shivs_Poll_WavFile $wav  (Required) The wav file to append.
     * @throws Shivs_Poll_WavFileException
     */
    public function appendWav(Shivs_Poll_WavFile $wav) {
        // basic checks
        if ($wav->getSampleRate() != $this->getSampleRate()) {
            throw new Shivs_Poll_WavFileException("Sample rate for wav files do not match.");
        } else if ($wav->getBitsPerSample() != $this->getBitsPerSample()) {
            throw new Shivs_Poll_WavFileException("Bits per sample for wav files do not match.");
        } else if ($wav->getNumChannels() != $this->getNumChannels()) {
            throw new Shivs_Poll_WavFileException("Number of channels for wav files do not match.");
        }
     ...
     *
     * @param Shivs_Poll_WavFile $wav  (Required) The Shivs_Poll_WavFile to mix.
     * @param float $normalizeThreshold  (Optional) See normalizeSample for an explanation.
     * @throws Shivs_Poll_WavFileException
     */
    public function mergeWav(Shivs_Poll_WavFile $wav, $normalizeThreshold = null) {
        return $this->filter(array(
            Shivs_Poll_WavFile::FILTER_MIX       => $wav,
            Shivs_Poll_WavFile::FILTER_NORMALIZE => $normalizeThreshold
        ));
    }
    ...
    public function degrade($quality = 1.0)
    {
        return $this->filter(self::FILTER_DEGRADE, array(
            Shivs_Poll_WavFile::FILTER_DEGRADE => $quality
        ));
    }
    ...
    public function convertBitsPerSample($bitsPerSample) {
        ...
        $tempWav = new Shivs_Poll_WavFile($this->getNumChannels(), $this->getSampleRate(), $bitsPerSample);
        $tempWav->filter(
...
/**
 * Shivs_Poll_WavFileException indicates an illegal state or argument in this class.
 */
class Shivs_Poll_WavFileException extends Exception {}

/**
 * Shivs_Poll_WavFormatException indicates a malformed or unsupported wav file header.
 */
class Shivs_Poll_WavFormatException extends Shivs_Poll_WavFileException {}
