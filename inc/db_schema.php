...
class Shivs_Poll_DbSchema {

	/**
	 * This function creates all poll tables
	 * @static
	 */
	public static function create_poll_database_tables() {
		...
		Shivs_Poll_DbSchema::create_poll_answers_table();
		Shivs_Poll_DbSchema::create_poll_custom_fields_table();
		Shivs_Poll_DbSchema::create_poll_meta_table();
		Shivs_Poll_DbSchema::create_poll_answer_meta_table();
		Shivs_Poll_DbSchema::create_poll_logs_table();
		Shivs_Poll_DbSchema::create_poll_voters_table();
		Shivs_Poll_DbSchema::create_poll_bans_table();
		Shivs_Poll_DbSchema::create_poll_votes_custom_fields_table();
		Shivs_Poll_DbSchema::create_poll_facebook_users_table();

		$opt_box_modal_options = array(
			'show' => 'yes',
			'last_show_date' => Shivs_Poll_Model::get_mysql_curent_date(),
			'modal_had_submit' => 'no',
			'sidebar_had_submit' => 'no'
		);

		// let's support professional stuffs some time
		$pro_options = array(
			'pro_key' => '',
			'pro_api_key' => '',
			'pro_api_server_url' => '',
		...
		$default_options = array(
			...
			'start_date'								=> Shivs_Poll_Model::get_mysql_curent_date(),
			...
			'poll_archive_url'							=> home_url().'/shivs-poll-archive',
			...
			'email_notifications_from_name'				=> 'Shivs Poll',
			'email_notifications_from_email'			=> 'shivs-poll@' . $sitename,
		...
		if ( ! get_option( "shivs_poll_version" ) ) {

			$poll_archive_page	= get_page_by_path('shivs-poll-archive', ARRAY_A );
			if ( ! $poll_archive_page ) {
				$_p = array();
				$_p['post_title'] = 'Shivs Poll Archive';
				$_p['post_content'] = "[shivs_poll_archive]";
			...
			$wpdb->query(
				"
				INSERT INTO `". $wpdb->shivs_polls . "` (`poll_author`, `name`, `question`, `start_date`, `end_date`, `total_votes`, `total_answers`, `status`, `date_added`, `last_modified`, `show_in_archive`) VALUES
				(". $poll_author .", 'Default Poll', 'How is my plugin?', '".current_time( 'mysql' )."', '9999-12-31 23:59:59', 0, 0, 'open', '".current_time( 'mysql' )."', '".current_time( 'mysql' )."', '".$default_options['show_in_archive']."')
				"
			);
			$poll_id = $wpdb->insert_id;
			$wpdb->query(
				"
				INSERT INTO `". $wpdb->shivs_poll_answers . "` (`poll_id`, `answer`, `type`, `votes`, `status`) VALUES
				(" . $poll_id . ", 'Good', 'default', 0, 'active'),
				(" . $poll_id . ", 'Better', 'default', 0, 'active'),
				(" . $poll_id . ", 'Best', 'default', 0, 'active'),
			...
			update_option( "shivs_poll_first_install_date", Shivs_Poll_Model::get_mysql_curent_date() );
			update_option( "shivs_poll_admin_notices_donate", 'yes' );
		}

		update_option( "shivs_poll_version", $wpdb->shivs_poll_version );
	} // END create_poll_database_tables

	/**
	 * Checks if the current template is existing in the database
	 * @param  string  $template_name The name of the template
	 * @return boolean                0 if not found
	 */
	public static function is_template_in_database( $template_name ) {

		global $wpdb;

		$is_template = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM ". $wpdb->shivs_poll_templates . " WHERE name = %s  LIMIT 0, 1", $template_name ) );

		return $is_template;
	}

	/**
	 * Adds a set of premade templates for the poll
	 * @param string $action        Add or Update action
	 * @param string $template_name The name of the template
	 */
	public static function add_update_templates_in_database( $action = 'add', $template_name ) {
		...
		if ( 'add' == $action ) {
			$sql  = "INSERT INTO `" . $wpdb->shivs_poll_templates . "` SET ";
...
<div id="shivs-poll-name-%POLL-ID%" class="shivs-poll-name">%POLL-NAME%</div>
...
<div id="shivs-poll-captcha-%POLL-ID%">
	<div class="shivs-poll-captcha-image-div" id="shivs-poll-captcha-image-div-%POLL-ID%">
		%CAPTCHA-IMAGE%       
		<div class="shivs-poll-captcha-helpers-div" id="shivs-poll-captcha-helpers-div-%POLL-ID%">
...
NOWDOC;
		...
		$wpdb->query( $sql );
	} // END add_update_templates_in_database

	public static function create_polls_table() {

		global $wpdb;

		$create_poll_table_sql = "CREATE TABLE " . $wpdb->shivs_polls . " (
	...
	public static function create_polls_templates_table() {
		...
		$create_poll_table_sql = "CREATE TABLE " . $wpdb->shivs_poll_templates . " (
		id int(11) NOT NULL AUTO_INCREMENT,
	...
	public static function create_poll_answers_table() {
		...
		$create_poll_answers_table_sql = "CREATE TABLE " .$wpdb->shivs_poll_answers . " (
		id int(11) NOT NULL AUTO_INCREMENT,
	...
	public static function create_poll_custom_fields_table() {
		...
		$create_poll_answers_table_sql = "CREATE TABLE " . $wpdb->shivs_poll_custom_fields . " (
		id int(11) NOT NULL AUTO_INCREMENT,
		poll_id int(11) NOT NULL,
	...
	public static function create_poll_meta_table() {
		...
		$create_poll_meta_table_sql = "CREATE TABLE " . $wpdb->shivs_pollmeta . " (
		meta_id int(11) NOT NULL AUTO_INCREMENT,
		shivs_poll_id int(11) NOT NULL,
		meta_key varchar(255) NOT NULL,
		meta_value longtext NOT NULL,
		PRIMARY KEY  (meta_id),
		KEY shivs_poll_id (shivs_poll_id)) CHARSET=".self::getCharset()." ;";
	...
	public static function create_poll_answer_meta_table() {
		...
		$create_poll_answer_meta_table_sql = "CREATE TABLE " . $wpdb->shivs_poll_answermeta . " (
		meta_id int(11) NOT NULL AUTO_INCREMENT,
		shivs_poll_answer_id int(11) NOT NULL,
		meta_key varchar(255) NOT NULL,
		meta_value longtext NOT NULL,
		PRIMARY KEY  (meta_id),
		KEY shivs_poll_answer_id (shivs_poll_answer_id)) CHARSET=".self::getCharset()." ;";
	...
	public static function create_poll_logs_table() {
		...
		$create_poll_logs_table_sql = "CREATE TABLE " . $wpdb->shivs_poll_logs . " (
		id int(11) NOT NULL AUTO_INCREMENT,
		poll_id int(11) NOT NULL,
	...
	public static function create_poll_voters_table() {
		...
		$create_poll_voters_table_sql = "CREATE TABLE " . $wpdb->shivs_poll_voters . " (
		id int(11) NOT NULL AUTO_INCREMENT,
		poll_id int(11) NOT NULL,
		user_id int(11) NOT NULL,
		user_type ENUM( 'facebook', 'wordpress', 'anonymous', 'default' ) NOT NULL DEFAULT 'default',
	...
	public static function create_poll_bans_table() {
		...
		$create_poll_bans_table_sql = "CREATE TABLE " . $wpdb->shivs_poll_bans . " (
		id int(11) NOT NULL AUTO_INCREMENT,
		poll_id int(11) NOT NULL,
	...
	public static function create_poll_votes_custom_fields_table() {
		...
		$create_poll_votes_custom_fields_table_sql = "CREATE TABLE " . $wpdb->shivs_poll_votes_custom_fields . " (
		id int(11) NOT NULL AUTO_INCREMENT,
		poll_id int(11) NOT NULL,
		vote_id varchar(255) NOT NULL,
	...
	public static function create_poll_facebook_users_table() {
		...
		$create_poll_table_sql = "CREATE TABLE " . $wpdb->shivs_poll_facebook_users . " (
		id int(11) NOT NULL AUTO_INCREMENT,
