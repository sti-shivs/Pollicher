...
/**
 * SHIVS Facebook Connect Model
 */
class Shivs_Poll_Facebook_Connect_Model {
	...
	public static function getInstance( ) {
		if ( !isset( self::$_instance ) )
			self::$_instance = new Shivs_Poll_Facebook_Connect_Model( );
		return self::$_instance;
	}

	public function Shivs_Poll_Facebook_Connect_Model( ) {
		self::$_instance 	= $this;
	}

	private function load_facebook_credentials( $test = false ) {
		$this->facebook_credentials	= get_option( 'shivs_poll_facebook_credentials', false );
		if ( $test ) {
            ...
            if ( 'true'	=== $temp['success'] ) {
				...
				$this->error	= __( 'Sorry! Connection with server failed!', 'shivs_poll' );
				return false;
			}
		}
	}
