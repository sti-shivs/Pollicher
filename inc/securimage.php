...
class Shivs_Poll_Securimage {
     ...
     * @var Shivs_Poll_Securimage_Color
     */
     ...
     * <code>
     * $options = array(
     *     'text_color' => new Shivs_Poll_Securimage_Color('#013020'),
     *     'code_length' => 5,
     *     'num_lines' => 5,
     *     'noise_level' => 3,
    ...
    public function __construct($options = array())
    {
        ...
        if (is_array($options) && sizeof($options) > 0) {
            foreach($options as $prop => $val) {
                if ($prop == 'captchaId') {
                    Shivs_Poll_Securimage::$_captchaId = $val;
    ...
    public static function getCaptchaId($new = true, array $options = array())
    {
        if ((bool)$new == true) {
            ...
            $si = new self($opts);
            Shivs_Poll_Securimage::$_captchaId = $id;
            $si->createCode();

            return $id;
        } else {
            return Shivs_Poll_Securimage::$_captchaId;
        }
    }
    ...
    */
    ...
    protected function getCodeFromDatabase()
    {
        ...
        if ($this->use_sqlite_db && $this->sqlite_handle !== false) {
            if (Shivs_Poll_Securimage::$_captchaId !== null) {
                $query = "SELECT * FROM codes WHERE id = '" . sqlite_escape_string(Shivs_Poll_Securimage::$_captchaId) . "'";
            } else {
                $ip = $_SERVER['REMOTE_ADDR'];
    ...
    {
        $wavCaptcha = new Shivs_Poll_WavFile();
        ...
        foreach ($letters as $letter) {
            ...
            try {
                $l = new Shivs_Poll_WavFile($this->audio_path . '/' . $letter . '.wav');
        ...
        if ($this->audio_use_noise == true) {
            ...
            if ($noiseFile !== false && is_readable($noiseFile)) {
                try {
                    $wavNoise = new Shivs_Poll_WavFile($noiseFile, false);
                } catch(Exception $ex) {
                    throw $ex;
                }

                // start at a random offset from the beginning of the Shivs_Poll_WavFile
                ...
                $filters[Shivs_Poll_WavFile::FILTER_MIX]       = $mixOpts;
                $filters[Shivs_Poll_WavFile::FILTER_NORMALIZE] = $this->audio_mix_normalization;
            }
        }
        ...
        if ($this->degrade_audio == true) {
            ...
            $filters[Shivs_Poll_WavFile::FILTER_DEGRADE] = rand(95, 98) / 100.0;
        }
    ...
    /**
     * Convert an html color code to a Shivs_Poll_Securimage_Color
     * @param string $color
     * @param Shivs_Poll_Securimage_Color $default The defalt color to use if $color is invalid
     */
    protected function initColor($color, $default)
    {
        if ($color == null) {
            return new Shivs_Poll_Securimage_Color($default);
        } else if (is_string($color)) {
            try {
                return new Shivs_Poll_Securimage_Color($color);
            } catch(Exception $e) {
                return new Shivs_Poll_Securimage_Color($default);
            }
        } else if (is_array($color) && sizeof($color) == 3) {
            return new Shivs_Poll_Securimage_Color($color[0], $color[1], $color[2]);
        } else {
            return new Shivs_Poll_Securimage_Color($default);
        }
    }
 ...
 *
 */
class Shivs_Poll_Securimage_Color
{
    public $r;
    public $g;
    public $b;
    ...
    /**
     * Create a new Shivs_Poll_Securimage_Color object.<br />
     ...
     * $color = new Shivs_Poll_Securimage_Color('#0080FF') or <br />
     * $color = new Shivs_Poll_Securimage_Color(0, 128, 255)
     *
     * @param string $color
     * @throws Exception
     */
        ...
        if (sizeof($args) == 0) {
            ...
            if (strlen($color) != 3 && strlen($color) != 6) {
                throw new InvalidArgumentException(
                  'Invalid HTML color code passed'
                );
            }
            ...
            throw new InvalidArgumentException(
              'Constructor expects 0, 1 or 3 arguments; ' . sizeof($args) . ' given'
            );
        }
    }
