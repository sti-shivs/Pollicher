...
class Shivs_Poll_Model {
	...
	public function __construct( $poll_id = -99, $offset = 0 ) {
		...
		// do not load ID -99. You won't like it
		$poll = NULL;
		...
		// latest poll
			...
			$polls = self::get_shivs_polls_filter_search( 'date_added', 'desc' );
			$poll  = ( $polls[$offset] ) ? $polls[$offset] : NULL;
		}

		// random poll
		else if ( -3 == $poll_id ) {
			$polls = self::get_shivs_polls_filter_search( 'rand()', '' );
			$poll  = $polls[0];
		}
		...
		if ( $poll ) {
			...
			$this->poll         = $poll;
			$this->poll_options = get_shivs_poll_meta( $this->poll['id'], 'options', true );
			$default_options    = get_option( 'shivs_poll_options', false );
	...
	public static function get_poll_options_by_id( $poll_id = 0 ) {
		...
		$poll_options    = get_shivs_poll_meta( $poll_id, 'options', true );
		$default_options = get_option( 'shivs_poll_options', false );
	...
	private function countAnswers( $request = array() ) {
		$answers = 0;
		if ( isset( $request['shivs_poll_answer'] ) ){
			$answers = count( $request['shivs_poll_answer'] );
		}
		if ( isset( $request['shivs_poll_options']['allow_other_answers'] ) ){
			if ( 'yes' == $request['shivs_poll_options']['allow_other_answers'] ){
				$answers = $answers + 1;
			}
		}
		return $answers;
	...
	public function verify_request_data( $request = array(), $config = NULL ) {
		...
		if ( isset( $request['shivs_poll_name'] ) && '' == trim( $request['shivs_poll_name'] ) ){
			$this->error = __( 'Poll name needed!', 'shivs_poll' );
			return false;
		}
		else if ( isset( $request['shivs_poll_question'] ) && '' == trim( $request['shivs_poll_question'] ) ) {
			$this->error = __( 'Poll question needed!', 'shivs_poll' );
			return false;
		}
		else if ( $this->countAnswers( $request ) < $config->min_number_of_answers ) {
			$this->error = __( 'At least ' . $config->min_number_of_answers . ' answers needed!', 'shivs_poll' );
			return false;
		}
		else {
			if ( isset( $request['shivs_poll_answer'] ) ){
				$index = 1;
				foreach ( $request['shivs_poll_answer'] as $answer_id => $answer_value ) {
					if ( '' == trim( $answer_value ) ){
						$this->error = __( 'Answer ' . $index . ' should not be empty!', 'shivs_poll' );
						return false;
					}
					$index++;
				}
			}
			else if ( isset( $request['shivs_poll_options']['allow_other_answers'] ) ) {
				if ( 'no' == $request['shivs_poll_options']['allow_other_answers'] ){
					$this->error = __( 'Answers not found!', 'shivs_poll' );
					return false;
				}
			}

			if ( isset( $request['shivs_poll_customfield'] ) ){
				$index = 1;
				if ( count( $request['shivs_poll_customfield'] > 0 ) ){
					foreach ( $request['shivs_poll_customfield'] as $customfield_id => $customfield_value ) {
						if ( '' == trim( $customfield_value ) ){
							$this->error = __( 'Custom Field ' . $index . ' should not be empty!', 'shivs_poll' );
							return false;
						}
						$index++;
					}
			...
			if ( isset( $request['shivs_poll_options']['start_date'] ) ){
				if ( '' == $request['shivs_poll_options']['start_date'] ){
					$this->error = __( 'Start Date needed!', 'shivs_poll' );
					return false;
				}
			}
			else {
				$this->error = __( 'Start Date not found!', 'shivs_poll' );
				return false;
			}

			if ( !isset( $request['shivs_poll_options']['never_expire'] ) ){
				if ( isset( $request['shivs_poll_options']['end_date'] ) ){
					if ( '' == $request['shivs_poll_options']['end_date'] ){
						$this->error = __( 'End Date needed!', 'shivs_poll' );
						return false;
					}
				}
				if ( isset( $request['shivs_poll_options']['start_date'] ) ){
					if ( isset( $request['shivs_poll_options']['end_date'] ) ){
						if ( $request['shivs_poll_options']['start_date'] > $request['shivs_poll_options']['end_date'] ){
							$this->error = __( 'Invalid entry! Start Date is after the  End Date! ', 'shivs_poll' );
							return false;
						}
					}
				}
			...
			if ( isset( $request['shivs_poll_options']['answer_result_label'] ) ){
				if ( 'votes-number' == $request['shivs_poll_options']['view_results_type'] ){
					if ( stripos( $request['shivs_poll_options']['answer_result_label'], '%POLL-ANSWER-RESULT-VOTES%' ) === false ){
						$this->error = __( 'Option "Poll Answer Result Label" Not Updated! You must use %POLL-ANSWER-RESULT-VOTES%!', 'shivs_poll' );
						return false;
					}
				}

				if ( 'percentages' == $request['shivs_poll_options']['view_results_type'] ){
					if ( stripos( $request['shivs_poll_options']['answer_result_label'], '%POLL-ANSWER-RESULT-PERCENTAGES%' ) === false ){
						$this->error = __( 'Option "Poll Answer Result Label" Not Updated! You must use %POLL-ANSWER-RESULT-PERCENTAGES%!', 'shivs_poll' );
						return false;
					}
				}

				if ( 'votes-number-and-percentages' == $request['shivs_poll_options']['view_results_type'] ){
					if ( stripos( $request['shivs_poll_options']['answer_result_label'], '%POLL-ANSWER-RESULT-PERCENTAGES%' ) === false ){
						$this->error = __( 'Option "Poll Answer Result Label" Not Updated! You must use %POLL-ANSWER-RESULT-VOTES% and %POLL-ANSWER-RESULT-PERCENTAGES%!', 'shivs_poll' );
						return false;
					}
					elseif ( stripos( $request['shivs_poll_options']['answer_result_label'], '%POLL-ANSWER-RESULT-VOTES%' ) === false ) {
						$this->error = __( 'Option "Poll Answer Result Label" Not Updated! You must use %POLL-ANSWER-RESULT-VOTES% and %POLL-ANSWER-RESULT-PERCENTAGES%!', 'shivs_poll' );
						return false;
					}
				}
				...
				$this->error = __( 'Option "Poll Answer Result Label" Not Updated!', 'shivs_poll' );
				return false;
			}

			if ( isset( $request['shivs_poll_options']['view_results_link'] ) ){
				if ( in_array( $request['shivs_poll_options']['view_results_link'], array( 'yes', 'no' ) ) ){
					if ( 'yes' == $request['shivs_poll_options']['view_results_link'] ){
						if ( isset( $request['shivs_poll_options']['view_results_link_label'] ) ){
							if ( '' == $request['shivs_poll_options']['view_results_link_label'] ){
								$this->error = __( 'Option "View Results Link Label" is empty!', 'shivs_poll' );
								return false;
							}
						}
						else {
							$this->error = __( 'Option "View Results Link Label" not found!', 'shivs_poll' );
							return false;
						}
					}
				}
				else {
					$this->error = __( 'Option "View Results Link" is invalid! You must choose between \'yes\' or \'no\'', 'shivs_poll' );
					return false;
				}
			}
			else {
				$this->error = __( 'Option "View Results Link" not found!', 'shivs_poll' );
				return false;
			}

			if ( isset( $request['shivs_poll_options']['view_back_to_vote_link'] ) ){
				if ( in_array( $request['shivs_poll_options']['view_back_to_vote_link'], array( 'yes', 'no' ) ) ){
					if ( 'yes' == $request['shivs_poll_options']['view_back_to_vote_link'] ){
						if ( isset( $request['shivs_poll_options']['view_back_to_vote_link_label'] ) ){
							if ( '' == $request['shivs_poll_options']['view_back_to_vote_link_label'] ){
								$this->error = __( 'Option "View Back to Vote Link Label" is empty!', 'shivs_poll' );
								return false;
							}
						}
						else {
							$this->error = __( 'Option "View Back to Vote Link Label" not found!', 'shivs_poll' );
							return false;
						}
					}
				}
				else {
					$this->error = __( 'Option "View Back to Vote Link" is invalid! You must choose between \'yes\' or \'no\'', 'shivs_poll' );
					return false;
				}
			}
			else {
				$this->error = __( 'Option "View Back to Vote Link" not found!', 'shivs_poll' );
				return false;
			}

			if ( isset( $request['shivs_poll_options']['view_total_votes'] ) ){
				if ( in_array( $request['shivs_poll_options']['view_total_votes'], array( 'yes', 'no' ) ) ){
					if ( 'yes' == $request['shivs_poll_options']['view_total_votes'] ){
						if ( isset( $request['shivs_poll_options']['view_total_votes_label'] ) ){
							if ( stripos( $request['shivs_poll_options']['view_total_votes_label'], '%POLL-TOTAL-VOTES%' ) === false ){
								$this->error = __( 'You must use %POLL-TOTAL-VOTES% to define your Total Votes label!', 'shivs_poll' );
								return false;
							}
						}
						else {
							$this->error = __( 'Option "Total Votes Label" not found!', 'shivs_poll' );
							return false;
						}
					}
				}
				else {
					$this->error = __( 'Option "View Total Votes" is invalid! Please choose between \'yes\' or \'no\'', 'shivs_poll' );
					return false;
				}
			}
			else {
				$this->error = __( 'Option "View Total Votes" not found!', 'shivs_poll' );
				return false;
			}

			if ( isset( $request['shivs_poll_options']['view_total_answers'] ) ){
				if ( in_array( $request['shivs_poll_options']['view_total_answers'], array( 'yes', 'no' ) ) ){
					if ( 'yes' == $request['shivs_poll_options']['view_total_answers'] ){
						if ( isset( $request['shivs_poll_options']['view_total_answers_label'] ) ){
							if ( stripos( $request['shivs_poll_options']['view_total_answers_label'], '%POLL-TOTAL-ANSWERS%' ) === false ){
								$this->error = __( 'You must use %POLL-TOTAL-ANSWERS% to define your Total Answers label!', 'shivs_poll' );
								return false;
							}
						}
						else {
							$this->error = __( 'Option "Total Answers Label" not found!', 'shivs_poll' );
							return false;
						}
					}
				}
				else {
					$this->error = __( 'Option "View Total Answers" is invalid! You must choose between \'yes\' or \'no\'', 'shivs_poll' );
					return false;
				}
			}
			else {
				$this->error = __( 'Option "View Total Answers" not found!', 'shivs_poll' );
				return false;
			}

			if ( isset( $request['shivs_poll_options']['view_poll_archive_link'] ) ){
				if ( in_array( $request['shivs_poll_options']['view_poll_archive_link'], array( 'yes', 'no' ) ) ){
					if ( 'yes' == $request['shivs_poll_options']['view_poll_archive_link'] ){
						if ( isset( $request['shivs_poll_options']['view_poll_archive_link_label'] ) ){
							if ( '' == $request['shivs_poll_options']['view_poll_archive_link_label'] ){
								$this->error = __( 'Option "View Poll Archive Link Label" is empty!', 'shivs_poll' );
								return false;
							}
						}
						else {
							$this->error = __( 'Option "View Poll Archive Link Label" not found!', 'shivs_poll' );
							return false;
						}

						if ( isset( $request['shivs_poll_options']['poll_archive_url'] ) ){
							if ( '' == $request['shivs_poll_options']['poll_archive_url'] ){
								$this->error = __( 'Option "Poll Archive URL" is empty!', 'shivs_poll' );
								return false;
							}
						}
						else {
							$this->error = __( 'Option "Poll Archive URL" not found!', 'shivs_poll' );
							return false;
						}
					}
				}
				else {
					$this->error = __( 'Option "View Poll Archive Link" Is Invalid! You must choose between \'yes\' or \'no\'', 'shivs_poll' );
					return false;
				}
			}
			else {
				$this->error = __( 'Option "View Poll Archive Link" not found!', 'shivs_poll' );
				return false;
			}

			if ( isset( $request['shivs_poll_options']['show_in_archive'] ) ){
				if ( in_array( $request['shivs_poll_options']['show_in_archive'], array( 'yes', 'no' ) ) ){
					if ( 'yes' == $request['shivs_poll_options']['show_in_archive'] ){
						if ( isset( $request['shivs_poll_options']['archive_order'] ) ){
							if ( '' == $request['shivs_poll_options']['archive_order'] ){
								$this->error = __( 'Option "Archive Order" is empty!', 'shivs_poll' );
								return false;
							}
						}
						else {
							$this->error = __( 'Option "Archive Order" not found!', 'shivs_poll' );
							return false;
						}
					}
				}
				else {
					$this->error = __( 'Option "Show in Archive" is invalid! You must choose between \'yes\' or \'no\'', 'shivs_poll' );
					return false;
				}
			}
			else {
				$this->error = __( 'Option "Show in Archive" not found!', 'shivs_poll' );
				return false;
			}

			if ( isset( $request['shivs_poll_options']['template'] ) ){
				$template = self::get_poll_template_from_database_by_id( $request['shivs_poll_options']['template'] );
				if ( !$template ){
					$this->error = __( 'Template not found!', 'shivs_poll' );
					return false;
				}
			}
			else {
				$this->error = __( 'You must choose a template for your poll!', 'shivs_poll' );
				return false;
			}
	...
	public function verify_template_request_data( $request = array(), $config = NULL ) {
		...
		if ( isset( $request['shivs_poll_template_name'] ) && '' == trim( $request['shivs_poll_template_name'] ) ){
			$this->error = __( 'Poll Template name needed!', 'shivs_poll' );
			return false;
		}
		elseif ( isset( $request['shivs_pol_template'] ) && '' == trim( $request['shivs_pol_template'] ) ) {
			$this->error = __( 'Poll template needed!', 'shivs_poll' );
			return false;
		}
		else {
			return true;
	...
	public function make_poll_template_from_request_data( $request = array(), $config = NULL ) {
		...
		$this->template['name']                       = isset( $request['shivs_poll_template_name'] ) ? trim( $request['shivs_poll_template_name'] ) : NULL;
		$this->template['before_vote_template']       = isset( $request['shivs_poll_before_vote_template'] ) ? trim( $request['shivs_poll_before_vote_template'] ) : NULL;
		$this->template['after_vote_template']        = isset( $request['shivs_poll_after_vote_template'] ) ? trim( $request['shivs_poll_after_vote_template'] ) : NULL;
		$this->template['before_start_date_template'] = isset( $request['shivs_poll_template_before_start_date'] ) ? trim( $request['shivs_poll_template_before_start_date'] ) : NULL;
		$this->template['after_end_date_template']    = isset( $request['shivs_poll_template_after_end_date'] ) ? trim( $request['shivs_poll_template_after_end_date'] ) : NULL;
		$this->template['css']                        = isset( $request['shivs_poll_template_css'] ) ? trim( $request['shivs_poll_template_css'] ) : NULL;
		$this->template['js']                         = isset( $request['shivs_poll_template_css'] ) ? trim( $request['shivs_poll_template_js'] ) : NULL;
		$this->template['reset_template_id']          = isset( $request['shivs_poll_reset_template_id'] ) ? trim( $request['shivs_poll_reset_template_id'] ) : NULL;
	}
	...
	public function add_poll_template_to_database( $request = array(), $config = NULL ) {
		...
		if ( $this->verify_template_request_data( $request, $config ) ) {
			...
			if ( !isset( $result['id'] ) ) {
				...
				$this->error = __( 'This template name already exist! Please choose another name!', 'shivs_poll' );
				return false;
			}
		}
		else {
	...
	public static function add_bans( $request ) {
		...
		if ( !isset( $request['ban_poll_id'] ) ) {
			...
			$error = __( 'You must choose a poll! ' );
		}
		else if ( !ctype_digit( $request['ban_poll_id'] ) ) {

			$error = __( 'Invalid poll! Please try again! ' );
		}
		else if ( !in_array( $request['ban_type'], array( 'ip', 'username', 'email' ) ) ) {

			$error = __( 'You must choose a ban type!', 'shivs_poll' );
		}
		else if ( '' == trim( $request['ban_value'] ) ) {

			$error = __( 'You must choose a ban value!', 'shivs_poll' );
		}
		else {
		...
		if ( $this->verify_template_request_data( $request, $config ) ) {
			...
			if ( isset( $result['id'] ) ) {
				...
				if ( !isset( $result['id'] ) ) {
					...
					$this->error = __( 'This template name already exists! Please choose another name!', 'shivs_poll' );
				...
				$this->error = __( 'This poll template doesn`t exist!', 'shivs_poll' );
	...
	public function reset_poll_template( $request = array(), $config = NULL ) {
		...
		if ( isset( $result['id'] ) ) {
			...
			if ( '' != $this->template['reset_template_id'] ) {
				...
				$this->error = __( 'You need to choose a template to reset!', 'shivs_poll' );
	...
	private static function reset_poll_template_in_database( $poll_id, $template_id ) {
		...
		$sql = "UPDATE `" . $wpdb->shivs_poll_templates . "` SET ";
		switch ( $template_id ) {
			case '1': //White template
				$sql .= "`before_vote_template` = '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
				`css`							= '#shivs-poll-container-%POLL-ID% {\r\n    width:%POLL-WIDTH%;\r\n    background:#fff;\r\n    padding:10px;\r\n    color:#555;\r\n    overflow:hidden;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-name-%POLL-ID% {\r\n    font-size:14px;\r\n    font-weight:bold;\r\n}\r\n\r\n#shivs-poll-question-%POLL-ID% {\r\n    font-size:14px;\r\n    margin:5px 0px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% {  }\r\n#shivs-poll-answers-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li { \r\n    font-style:normal;\r\n    margin:0px 0px 10px 0px;\r\n    padding:0px;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li input { \r\n    margin:0px; \r\n    float:none;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li label { \r\n    margin:0px; \r\n    font-style:normal; \r\n    font-weight:normal; \r\n    font-size:12px; \r\n    float:none;\r\n}\r\n.shivs-poll-results-%POLL-ID% {\r\n    font-size: 12px;\r\n    font-style: italic;\r\n    font-weight: normal;\r\n    margin-left: 15px;\r\n}\r\n\r\n#shivs-poll-custom-%POLL-ID% {  }\r\n#shivs-poll-custom-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-custom-%POLL-ID% ul li { \r\n    padding:0px;\r\n    margin:0px;    \r\n    font-size:14px;\r\n}\r\n#shivs-poll-container-%POLL-ID% input[type=\'text\'] { margin:0px 0px 5px 0px; padding:2%; width:96%; text-indent:2%; font-size:12px; }\r\n\r\n#shivs-poll-captcha-input-div-%POLL-ID% {\r\nmargin-top:5px;\r\n}\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% {\r\nwidth:30px;\r\nfloat:left;\r\nmargin-left:5px;\r\nheight:0px;\r\n}\r\n\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% img {\r\nmargin-bottom:2px;\r\n}\r\n\r\n#shivs-poll-captcha-image-div-%POLL-ID% {\r\nmargin-bottom:5px;\r\n}\r\n\r\n#shivs_poll_captcha_image_%POLL-ID% {\r\nfloat:left;\r\n}\r\n\r\n.shivs_poll_clear {\r\nclear:both;\r\n}\r\n\r\n#shivs-poll-vote-%POLL-ID% {\r\n\r\n}\r\n.shivs-poll-results-bar-%POLL-ID% { background:#f5f5f5; height:10px;  }\r\n.shivs-poll-results-bar-%POLL-ID% div { background:#555; height:10px; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-vote-%POLL-ID% button { float:left; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% {\r\n    float: right;\r\n    margin-bottom: 20px;\r\n    margin-top: -20px;\r\n    width: auto;\r\n}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% a { color:#555; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-back-%POLL-ID% a { color:#555; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div { float:left; width:100%; }\r\n\r\n#shivs-poll-container-error-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:red;\r\n    text-transform:lowercase;\r\n}\r\n\r\n#shivs-poll-container-success-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:green;\r\n}',
				`js`							= 'function stripBorder_%POLL-ID%(object) {\r\n    object.each(function() {\r\n            if( parseInt(jQuery(this).width() ) > 0) {\r\n                jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ")) );\r\n            }\r\n            else {\r\n                jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ", \'0px\');\r\n                jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ", \'0px\');\r\n            }\r\n    });\r\n}\r\nfunction stripPadding_%POLL-ID%(object) {\r\n    object.each(function() { \r\n            jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) );\r\n    });\r\n}\r\n\r\nfunction strip_results_%POLL-ID%() {\r\n        stripPadding_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs_poll_li_result-%POLL-ID%\\" . '"' . ") );   \r\n    stripBorder_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-result-bar-%POLL-ID%\\" . '"' . "));\r\n}\r\n\r\njQuery(document).ready(function(e) {\r\n                if(typeof window.strip_results_%POLL-ID% == \'function\') \r\n            strip_results_%POLL-ID%();\r\n    \r\n        if(typeof window.tabulate_answers_%POLL-ID% == \'function\') \r\n            tabulate_answers_%POLL-ID%();\r\n        \r\n        if(typeof window.tabulate_results_%POLL-ID% == \'function\') \r\n            tabulate_results_%POLL-ID%();\r\n        \r\n});\r\n\r\nfunction equalWidth_%POLL-ID%(obj, cols, findWidest ) {\r\n findWidest  = typeof findWidest  !== \'undefined\' ? findWidest  : false;\r\n    if ( findWidest ) {\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                width = parseInt(thisWidth / cols); \r\n                jQuery(this).width(width);    \r\n                jQuery(this).css(\'float\', \'left\');    \r\n        });\r\n    }\r\n    else {\r\n        var widest = 0;\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                if(thisWidth > widest) {\r\n                    widest = thisWidth; \r\n                }    \r\n        });\r\n        width = parseInt( widest / cols); \r\n        obj.width(width);    \r\n        obj.css(\'float\', \'left\');    \r\n    }    \r\n}\r\n\r\nfunction tabulate_answers_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID%\\" . '"' . "), %ANSWERS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %ANSWERS-TABULATED-COLS%, true );\r\n}\r\n\r\nfunction tabulate_results_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID%\\" . '"' . "), %RESULTS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %RESULTS-TABULATED-COLS%, true );\r\n}',
				`last_modified`					= '" . current_time( 'mysql' ) . "' ";
			break;

			case '2': //Grey template
				$sql .= "`before_vote_template` = '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
				`css`							= '#shivs-poll-container-%POLL-ID% {\r\n    width:%POLL-WIDTH%;\r\n    background:#eee;\r\n    padding:10px;\r\n    color:#000;\r\n    overflow:hidden;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-name-%POLL-ID% {\r\n    font-size:14px;\r\n    font-weight:bold;\r\n}\r\n\r\n#shivs-poll-question-%POLL-ID% {\r\n    font-size:14px;\r\n    margin:5px 0px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% {  }\r\n#shivs-poll-answers-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li { \r\n    font-style:normal;\r\n    margin:0px 0px 10px 0px;\r\n    padding:0px;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li input { \r\n    margin:0px; \r\n    float:none;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li label { \r\n    margin:0px; \r\n    font-style:normal; \r\n    font-weight:normal; \r\n    font-size:12px; \r\n    float:none;\r\n}\r\n.shivs-poll-results-%POLL-ID% {\r\n    font-size: 12px;\r\n    font-style: italic;\r\n    font-weight: normal;\r\n    margin-left: 15px;\r\n}\r\n\r\n#shivs-poll-custom-%POLL-ID% {  }\r\n#shivs-poll-custom-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-custom-%POLL-ID% ul li { \r\n    padding:0px;\r\n    margin:0px;    \r\n    font-size:14px;\r\n}\r\n#shivs-poll-container-%POLL-ID% input[type=\'text\'] { margin:0px 0px 5px 0px; padding:2%; width:96%; text-indent:2%; font-size:12px; }\r\n\r\n#shivs-poll-captcha-input-div-%POLL-ID% {\r\nmargin-top:5px;\r\n}\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% {\r\nwidth:30px;\r\nfloat:left;\r\nmargin-left:5px;\r\nheight:0px;\r\n}\r\n\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% img {\r\nmargin-bottom:2px;\r\n}\r\n\r\n#shivs-poll-captcha-image-div-%POLL-ID% {\r\nmargin-bottom:5px;\r\n}\r\n\r\n#shivs_poll_captcha_image_%POLL-ID% {\r\nfloat:left;\r\n}\r\n\r\n.shivs_poll_clear {\r\nclear:both;\r\n}\r\n\r\n#shivs-poll-vote-%POLL-ID% {\r\n\r\n}\r\n.shivs-poll-results-bar-%POLL-ID% { background:#f5f5f5; height:10px;  }\r\n.shivs-poll-results-bar-%POLL-ID% div { background:#555; height:10px; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-vote-%POLL-ID% button { float:left; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% {\r\n    float: right;\r\n    margin-bottom: 20px;\r\n    margin-top: -20px;\r\n    width: auto;\r\n}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% a { color:#000; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-back-%POLL-ID% a { color:#000; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div { float:left; width:100%; }\r\n\r\n#shivs-poll-container-error-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:red;\r\n    text-transform:lowercase;\r\n}\r\n\r\n#shivs-poll-container-success-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:green;\r\n}',
				`js`							= 'function stripBorder_%POLL-ID%(object) {\r\n    object.each(function() {\r\n            if( parseInt(jQuery(this).width() ) > 0) {\r\n                jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ")) );\r\n            }\r\n            else {\r\n                jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ", \'0px\');\r\n                jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ", \'0px\');\r\n            }\r\n    });\r\n}\r\nfunction stripPadding_%POLL-ID%(object) {\r\n    object.each(function() { \r\n            jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) );\r\n    });\r\n}\r\n\r\nfunction strip_results_%POLL-ID%() {\r\n        stripPadding_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs_poll_li_result-%POLL-ID%\\" . '"' . ") );   \r\n    stripBorder_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-result-bar-%POLL-ID%\\" . '"' . "));\r\n}\r\n\r\njQuery(document).ready(function(e) {\r\n                if(typeof window.strip_results_%POLL-ID% == \'function\') \r\n            strip_results_%POLL-ID%();\r\n    \r\n        if(typeof window.tabulate_answers_%POLL-ID% == \'function\') \r\n            tabulate_answers_%POLL-ID%();\r\n        \r\n        if(typeof window.tabulate_results_%POLL-ID% == \'function\') \r\n            tabulate_results_%POLL-ID%();\r\n        \r\n});\r\n\r\nfunction equalWidth_%POLL-ID%(obj, cols, findWidest ) {\r\n findWidest  = typeof findWidest  !== \'undefined\' ? findWidest  : false;\r\n    if ( findWidest ) {\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                width = parseInt(thisWidth / cols); \r\n                jQuery(this).width(width);    \r\n                jQuery(this).css(\'float\', \'left\');    \r\n        });\r\n    }\r\n    else {\r\n        var widest = 0;\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                if(thisWidth > widest) {\r\n                    widest = thisWidth; \r\n                }    \r\n        });\r\n        width = parseInt( widest / cols); \r\n        obj.width(width);    \r\n        obj.css(\'float\', \'left\');    \r\n    }    \r\n}\r\n\r\nfunction tabulate_answers_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID%\\" . '"' . "), %ANSWERS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %ANSWERS-TABULATED-COLS%, true );\r\n}\r\n\r\nfunction tabulate_results_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID%\\" . '"' . "), %RESULTS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %RESULTS-TABULATED-COLS%, true );\r\n}',
				`last_modified`					= '" . current_time( 'mysql' ) . "' ";
			break;

			case '3': //Dark template
				$sql .= "`before_vote_template` = '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
				`css`							= '#shivs-poll-container-%POLL-ID% {\r\n    width:%POLL-WIDTH%;\r\n    background:#555;\r\n    padding:10px;\r\n    color:#fff;\r\n    overflow:hidden;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-name-%POLL-ID% {\r\n    font-size:14px;\r\n    font-weight:bold;\r\n}\r\n\r\n#shivs-poll-question-%POLL-ID% {\r\n    font-size:14px;\r\n    margin:5px 0px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% {  }\r\n#shivs-poll-answers-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li { \r\n    font-style:normal;\r\n    margin:0px 0px 10px 0px;\r\n    padding:0px;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li input { \r\n    margin:0px; \r\n    float:none;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li label { \r\n    margin:0px; \r\n    font-style:normal; \r\n    font-weight:normal; \r\n    font-size:12px; \r\n    float:none;\r\n}\r\n.shivs-poll-results-%POLL-ID% {\r\n    font-size: 12px;\r\n    font-style: italic;\r\n    font-weight: normal;\r\n    margin-left: 15px;\r\n}\r\n\r\n#shivs-poll-custom-%POLL-ID% {  }\r\n#shivs-poll-custom-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-custom-%POLL-ID% ul li { \r\n    padding:0px;\r\n    margin:0px;    \r\n    font-size:14px;\r\n}\r\n#shivs-poll-container-%POLL-ID% input[type=\'text\'] { margin:0px 0px 5px 0px; padding:2%; width:96%; text-indent:2%; font-size:12px; }\r\n\r\n#shivs-poll-captcha-input-div-%POLL-ID% {\r\nmargin-top:5px;\r\n}\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% {\r\nwidth:30px;\r\nfloat:left;\r\nmargin-left:5px;\r\nheight:0px;\r\n}\r\n\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% img {\r\nmargin-bottom:2px;\r\n}\r\n\r\n#shivs-poll-captcha-image-div-%POLL-ID% {\r\nmargin-bottom:5px;\r\n}\r\n\r\n#shivs_poll_captcha_image_%POLL-ID% {\r\nfloat:left;\r\n}\r\n\r\n.shivs_poll_clear {\r\nclear:both;\r\n}\r\n\r\n#shivs-poll-vote-%POLL-ID% {\r\n\r\n}\r\n.shivs-poll-results-bar-%POLL-ID% { background:#f5f5f5; height:10px;  }\r\n.shivs-poll-results-bar-%POLL-ID% div { background:#333333; height:10px; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-vote-%POLL-ID% button { float:left; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% {\r\n    float: right;\r\n    margin-bottom: 20px;\r\n    margin-top: -20px;\r\n    width: auto;\r\n}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% a { color:#fff; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-back-%POLL-ID% a { color:#fff; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div { float:left; width:100%; }\r\n\r\n#shivs-poll-container-error-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:red;\r\n    text-transform:lowercase;\r\n}\r\n\r\n#shivs-poll-container-success-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:green;\r\n}',
				`js`							= 'function stripBorder_%POLL-ID%(object) {\r\n    object.each(function() {\r\n            if( parseInt(jQuery(this).width() ) > 0) {\r\n                jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ")) );\r\n            }\r\n            else {\r\n                jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ", \'0px\');\r\n                jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ", \'0px\');\r\n            }\r\n    });\r\n}\r\nfunction stripPadding_%POLL-ID%(object) {\r\n    object.each(function() { \r\n            jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) );\r\n    });\r\n}\r\n\r\nfunction strip_results_%POLL-ID%() {\r\n        stripPadding_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs_poll_li_result-%POLL-ID%\\" . '"' . ") );   \r\n    stripBorder_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-result-bar-%POLL-ID%\\" . '"' . "));\r\n}\r\n\r\njQuery(document).ready(function(e) {\r\n                if(typeof window.strip_results_%POLL-ID% == \'function\') \r\n            strip_results_%POLL-ID%();\r\n    \r\n        if(typeof window.tabulate_answers_%POLL-ID% == \'function\') \r\n            tabulate_answers_%POLL-ID%();\r\n        \r\n        if(typeof window.tabulate_results_%POLL-ID% == \'function\') \r\n            tabulate_results_%POLL-ID%();\r\n        \r\n});\r\n\r\nfunction equalWidth_%POLL-ID%(obj, cols, findWidest ) {\r\n findWidest  = typeof findWidest  !== \'undefined\' ? findWidest  : false;\r\n    if ( findWidest ) {\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                width = parseInt(thisWidth / cols); \r\n                jQuery(this).width(width);    \r\n                jQuery(this).css(\'float\', \'left\');    \r\n        });\r\n    }\r\n    else {\r\n        var widest = 0;\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                if(thisWidth > widest) {\r\n                    widest = thisWidth; \r\n                }    \r\n        });\r\n        width = parseInt( widest / cols); \r\n        obj.width(width);    \r\n        obj.css(\'float\', \'left\');    \r\n    }    \r\n}\r\n\r\nfunction tabulate_answers_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID%\\" . '"' . "), %ANSWERS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %ANSWERS-TABULATED-COLS%, true );\r\n}\r\n\r\nfunction tabulate_results_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID%\\" . '"' . "), %RESULTS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %RESULTS-TABULATED-COLS%, true );\r\n}',
				`last_modified`					= '" . current_time( 'mysql' ) . "' ";
			break;

			case '4': //Blue 1.0 template
				$sql .= "`before_vote_template` = '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
			...
			case '5': //Blue 2.0 template
				$sql .= "`before_vote_template` = '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
				`css`							= '#shivs-poll-container-%POLL-ID% {\r\n    width:%POLL-WIDTH%;\r\n    background:#fff;\r\n    padding:0px;\r\n    color:#555;\r\n    overflow:hidden;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-name-%POLL-ID% {\r\n    font-weight:bold;\r\n    background:#327BD6;\r\n    color:#fff;\r\n    padding:5px;\r\n        text-align:center;\r\n        font-size:12px;\r\n}\r\n\r\n#shivs-poll-question-%POLL-ID% {\r\n    font-size:14px;\r\n    margin:5px 0px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% {  }\r\n#shivs-poll-answers-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li { \r\n    font-style:normal;\r\n    margin:0px 0px 10px 0px;\r\n    padding:0px;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li input { \r\n    margin:0px; \r\n    float:none;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li label { \r\n    margin:0px; \r\n    font-style:normal; \r\n    font-weight:normal; \r\n    font-size:12px; \r\n    float:none;\r\n}\r\n.shivs-poll-results-%POLL-ID% {\r\n    font-size: 12px;\r\n    font-style: italic;\r\n    font-weight: normal;\r\n    margin-left: 15px;\r\n}\r\n\r\n#shivs-poll-custom-%POLL-ID% {  }\r\n#shivs-poll-custom-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-custom-%POLL-ID% ul li { \r\n    padding:0px;\r\n    margin:0px;    \r\n    font-size:14px;\r\n}\r\n#shivs-poll-container-%POLL-ID% input[type=\'text\'] { margin:0px 0px 5px 0px; padding:2%; width:96%; text-indent:2%; font-size:12px; }\r\n\r\n#shivs-poll-captcha-input-div-%POLL-ID% {\r\nmargin-top:5px;\r\n}\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% {\r\nwidth:30px;\r\nfloat:left;\r\nmargin-left:5px;\r\nheight:0px;\r\n}\r\n\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% img {\r\nmargin-bottom:2px;\r\n}\r\n\r\n#shivs-poll-captcha-image-div-%POLL-ID% {\r\nmargin-bottom:5px;\r\n}\r\n\r\n#shivs_poll_captcha_image_%POLL-ID% {\r\nfloat:left;\r\n}\r\n\r\n.shivs_poll_clear {\r\nclear:both;\r\n}\r\n\r\n#shivs-poll-vote-%POLL-ID% {\r\n\r\n}\r\n.shivs-poll-results-bar-%POLL-ID% { background:#f5f5f5; height:10px;  }\r\n.shivs-poll-results-bar-%POLL-ID% div { background:#555; height:10px; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-vote-%POLL-ID% button { float:left; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% {\r\n    float: right;\r\n    margin-bottom: 20px;\r\n    margin-top: -20px;\r\n    width: auto;\r\n}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% a { color:#327BD6; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-back-%POLL-ID% a { color:#327BD6; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div { float:left; width:100%; }\r\n\r\n#shivs-poll-container-error-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:red;\r\n    text-transform:lowercase;\r\n}\r\n\r\n#shivs-poll-container-success-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:green;\r\n}',
				`js`							= 'function stripBorder_%POLL-ID%(object) {\r\n    object.each(function() {\r\n            if( parseInt(jQuery(this).width() ) > 0) {\r\n                jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ")) );\r\n            }\r\n            else {\r\n                jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ", \'0px\');\r\n                jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ", \'0px\');\r\n            }\r\n    });\r\n}\r\nfunction stripPadding_%POLL-ID%(object) {\r\n    object.each(function() { \r\n            jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) );\r\n    });\r\n}\r\n\r\nfunction strip_results_%POLL-ID%() {\r\n        stripPadding_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs_poll_li_result-%POLL-ID%\\" . '"' . ") );   \r\n    stripBorder_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-result-bar-%POLL-ID%\\" . '"' . "));\r\n}\r\n\r\njQuery(document).ready(function(e) {\r\n                if(typeof window.strip_results_%POLL-ID% == \'function\') \r\n            strip_results_%POLL-ID%();\r\n    \r\n        if(typeof window.tabulate_answers_%POLL-ID% == \'function\') \r\n            tabulate_answers_%POLL-ID%();\r\n        \r\n        if(typeof window.tabulate_results_%POLL-ID% == \'function\') \r\n            tabulate_results_%POLL-ID%();\r\n        \r\n});\r\n\r\nfunction equalWidth_%POLL-ID%(obj, cols, findWidest ) {\r\n findWidest  = typeof findWidest  !== \'undefined\' ? findWidest  : false;\r\n    if ( findWidest ) {\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                width = parseInt(thisWidth / cols); \r\n                jQuery(this).width(width);    \r\n                jQuery(this).css(\'float\', \'left\');    \r\n        });\r\n    }\r\n    else {\r\n        var widest = 0;\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                if(thisWidth > widest) {\r\n                    widest = thisWidth; \r\n                }    \r\n        });\r\n        width = parseInt( widest / cols); \r\n        obj.width(width);    \r\n        obj.css(\'float\', \'left\');    \r\n    }    \r\n}\r\n\r\nfunction tabulate_answers_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID%\\" . '"' . "), %ANSWERS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %ANSWERS-TABULATED-COLS%, true );\r\n}\r\n\r\nfunction tabulate_results_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID%\\" . '"' . "), %RESULTS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %RESULTS-TABULATED-COLS%, true );\r\n}',
				`last_modified`					= '" . current_time( 'mysql' ) . "' ";
			break;

			case '6': //Blue 3.0 template
				$sql .= "`before_vote_template`	= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
			...
			case '8': //Red 2.0 template
				$sql .= "`before_vote_template`	= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
				`css`							= '#shivs-poll-container-%POLL-ID% {\r\n    width:%POLL-WIDTH%;\r\n    background:#fff;\r\n    padding:0px;\r\n    color:#555;\r\n    overflow:hidden;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-name-%POLL-ID% {\r\n    font-weight:bold;\r\n    background:#B70004;\r\n    color:#fff;\r\n    padding:5px;\r\n        text-align:center;\r\n        font-size:12px;\r\n}\r\n\r\n#shivs-poll-question-%POLL-ID% {\r\n    font-size:14px;\r\n    margin:5px 0px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% {  }\r\n#shivs-poll-answers-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li { \r\n    font-style:normal;\r\n    margin:0px 0px 10px 0px;\r\n    padding:0px;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li input { \r\n    margin:0px; \r\n    float:none;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li label { \r\n    margin:0px; \r\n    font-style:normal; \r\n    font-weight:normal; \r\n    font-size:12px; \r\n    float:none;\r\n}\r\n.shivs-poll-results-%POLL-ID% {\r\n    font-size: 12px;\r\n    font-style: italic;\r\n    font-weight: normal;\r\n    margin-left: 15px;\r\n}\r\n\r\n#shivs-poll-custom-%POLL-ID% {  }\r\n#shivs-poll-custom-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-custom-%POLL-ID% ul li { \r\n    padding:0px;\r\n    margin:0px;    \r\n    font-size:14px;\r\n}\r\n#shivs-poll-container-%POLL-ID% input[type=\'text\'] { margin:0px 0px 5px 0px; padding:2%; width:96%; text-indent:2%; font-size:12px; }\r\n\r\n#shivs-poll-captcha-input-div-%POLL-ID% {\r\nmargin-top:5px;\r\n}\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% {\r\nwidth:30px;\r\nfloat:left;\r\nmargin-left:5px;\r\nheight:0px;\r\n}\r\n\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% img {\r\nmargin-bottom:2px;\r\n}\r\n\r\n#shivs-poll-captcha-image-div-%POLL-ID% {\r\nmargin-bottom:5px;\r\n}\r\n\r\n#shivs_poll_captcha_image_%POLL-ID% {\r\nfloat:left;\r\n}\r\n\r\n.shivs_poll_clear {\r\nclear:both;\r\n}\r\n\r\n#shivs-poll-vote-%POLL-ID% {\r\n\r\n}\r\n.shivs-poll-results-bar-%POLL-ID% { background:#f5f5f5; height:10px;  }\r\n.shivs-poll-results-bar-%POLL-ID% div { background:#555; height:10px; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-vote-%POLL-ID% button { float:left; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% {\r\n    float: right;\r\n    margin-bottom: 20px;\r\n    margin-top: -20px;\r\n    width: auto;\r\n}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% a { color:#B70004; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-back-%POLL-ID% a { color:#B70004; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div { float:left; width:100%; }\r\n\r\n#shivs-poll-container-error-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:red;\r\n    text-transform:lowercase;\r\n}\r\n\r\n#shivs-poll-container-success-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:green;\r\n}',
				`js`							= 'function stripBorder_%POLL-ID%(object) {\r\n    object.each(function() {\r\n            if( parseInt(jQuery(this).width() ) > 0) {\r\n                jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ")) );\r\n            }\r\n            else {\r\n                jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ", \'0px\');\r\n                jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ", \'0px\');\r\n            }\r\n    });\r\n}\r\nfunction stripPadding_%POLL-ID%(object) {\r\n    object.each(function() { \r\n            jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) );\r\n    });\r\n}\r\n\r\nfunction strip_results_%POLL-ID%() {\r\n        stripPadding_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs_poll_li_result-%POLL-ID%\\" . '"' . ") );   \r\n    stripBorder_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-result-bar-%POLL-ID%\\" . '"' . "));\r\n}\r\n\r\njQuery(document).ready(function(e) {\r\n                if(typeof window.strip_results_%POLL-ID% == \'function\') \r\n            strip_results_%POLL-ID%();\r\n    \r\n        if(typeof window.tabulate_answers_%POLL-ID% == \'function\') \r\n            tabulate_answers_%POLL-ID%();\r\n        \r\n        if(typeof window.tabulate_results_%POLL-ID% == \'function\') \r\n            tabulate_results_%POLL-ID%();\r\n        \r\n});\r\n\r\nfunction equalWidth_%POLL-ID%(obj, cols, findWidest ) {\r\n findWidest  = typeof findWidest  !== \'undefined\' ? findWidest  : false;\r\n    if ( findWidest ) {\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                width = parseInt(thisWidth / cols); \r\n                jQuery(this).width(width);    \r\n                jQuery(this).css(\'float\', \'left\');    \r\n        });\r\n    }\r\n    else {\r\n        var widest = 0;\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                if(thisWidth > widest) {\r\n                    widest = thisWidth; \r\n                }    \r\n        });\r\n        width = parseInt( widest / cols); \r\n        obj.width(width);    \r\n        obj.css(\'float\', \'left\');    \r\n    }    \r\n}\r\n\r\nfunction tabulate_answers_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID%\\" . '"' . "), %ANSWERS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %ANSWERS-TABULATED-COLS%, true );\r\n}\r\n\r\nfunction tabulate_results_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID%\\" . '"' . "), %RESULTS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %RESULTS-TABULATED-COLS%, true );\r\n}',
				`last_modified`					= '" . current_time( 'mysql' ) . "' ";
			break;

			case '9': //Red 3.0 template
				$sql .= "`before_vote_template`	= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
			...
			case '11': //Green 2.0 template
				$sql .= "`before_vote_template`	= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
				`css`							= '#shivs-poll-container-%POLL-ID% {\r\n    width:%POLL-WIDTH%;\r\n    background:#fff;\r\n    padding:0px;\r\n    color:#555;\r\n    overflow:hidden;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-name-%POLL-ID% {\r\n    font-weight:bold;\r\n    background:#3F8B43;\r\n    color:#fff;\r\n    padding:5px;\r\n        text-align:center;\r\n        font-size:12px;\r\n}\r\n\r\n#shivs-poll-question-%POLL-ID% {\r\n    font-size:14px;\r\n    margin:5px 0px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% {  }\r\n#shivs-poll-answers-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li { \r\n    font-style:normal;\r\n    margin:0px 0px 10px 0px;\r\n    padding:0px;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li input { \r\n    margin:0px; \r\n    float:none;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li label { \r\n    margin:0px; \r\n    font-style:normal; \r\n    font-weight:normal; \r\n    font-size:12px; \r\n    float:none;\r\n}\r\n.shivs-poll-results-%POLL-ID% {\r\n    font-size: 12px;\r\n    font-style: italic;\r\n    font-weight: normal;\r\n    margin-left: 15px;\r\n}\r\n\r\n#shivs-poll-custom-%POLL-ID% {  }\r\n#shivs-poll-custom-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-custom-%POLL-ID% ul li { \r\n    padding:0px;\r\n    margin:0px;    \r\n    font-size:14px;\r\n}\r\n#shivs-poll-container-%POLL-ID% input[type=\'text\'] { margin:0px 0px 5px 0px; padding:2%; width:96%; text-indent:2%; font-size:12px; }\r\n\r\n#shivs-poll-captcha-input-div-%POLL-ID% {\r\nmargin-top:5px;\r\n}\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% {\r\nwidth:30px;\r\nfloat:left;\r\nmargin-left:5px;\r\nheight:0px;\r\n}\r\n\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% img {\r\nmargin-bottom:2px;\r\n}\r\n\r\n#shivs-poll-captcha-image-div-%POLL-ID% {\r\nmargin-bottom:5px;\r\n}\r\n\r\n#shivs_poll_captcha_image_%POLL-ID% {\r\nfloat:left;\r\n}\r\n\r\n.shivs_poll_clear {\r\nclear:both;\r\n}\r\n\r\n#shivs-poll-vote-%POLL-ID% {\r\n\r\n}\r\n.shivs-poll-results-bar-%POLL-ID% { background:#f5f5f5; height:10px;  }\r\n.shivs-poll-results-bar-%POLL-ID% div { background:#555; height:10px; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-vote-%POLL-ID% button { float:left; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% {\r\n    float: right;\r\n    margin-bottom: 20px;\r\n    margin-top: -20px;\r\n    width: auto;\r\n}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% a { color:#3F8B43; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-back-%POLL-ID% a { color:#3F8B43; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div { float:left; width:100%; }\r\n\r\n#shivs-poll-container-error-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:red;\r\n    text-transform:lowercase;\r\n}\r\n\r\n#shivs-poll-container-success-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:green;\r\n}',
				`js`							= 'function stripBorder_%POLL-ID%(object) {\r\n    object.each(function() {\r\n            if( parseInt(jQuery(this).width() ) > 0) {\r\n                jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ")) );\r\n            }\r\n            else {\r\n                jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ", \'0px\');\r\n                jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ", \'0px\');\r\n            }\r\n    });\r\n}\r\nfunction stripPadding_%POLL-ID%(object) {\r\n    object.each(function() { \r\n            jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) );\r\n    });\r\n}\r\n\r\nfunction strip_results_%POLL-ID%() {\r\n        stripPadding_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs_poll_li_result-%POLL-ID%\\" . '"' . ") );   \r\n    stripBorder_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-result-bar-%POLL-ID%\\" . '"' . "));\r\n}\r\n\r\njQuery(document).ready(function(e) {\r\n                if(typeof window.strip_results_%POLL-ID% == \'function\') \r\n            strip_results_%POLL-ID%();\r\n    \r\n        if(typeof window.tabulate_answers_%POLL-ID% == \'function\') \r\n            tabulate_answers_%POLL-ID%();\r\n        \r\n        if(typeof window.tabulate_results_%POLL-ID% == \'function\') \r\n            tabulate_results_%POLL-ID%();\r\n        \r\n});\r\n\r\nfunction equalWidth_%POLL-ID%(obj, cols, findWidest ) {\r\n findWidest  = typeof findWidest  !== \'undefined\' ? findWidest  : false;\r\n    if ( findWidest ) {\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                width = parseInt(thisWidth / cols); \r\n                jQuery(this).width(width);    \r\n                jQuery(this).css(\'float\', \'left\');    \r\n        });\r\n    }\r\n    else {\r\n        var widest = 0;\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                if(thisWidth > widest) {\r\n                    widest = thisWidth; \r\n                }    \r\n        });\r\n        width = parseInt( widest / cols); \r\n        obj.width(width);    \r\n        obj.css(\'float\', \'left\');    \r\n    }    \r\n}\r\n\r\nfunction tabulate_answers_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID%\\" . '"' . "), %ANSWERS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %ANSWERS-TABULATED-COLS%, true );\r\n}\r\n\r\nfunction tabulate_results_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID%\\" . '"' . "), %RESULTS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %RESULTS-TABULATED-COLS%, true );\r\n}',
				`last_modified`					= '" . current_time( 'mysql' ) . "' ";
			break;

			case '12': //Green 3.0 template
				$sql .= "`before_vote_template`	= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
			...
			case '14': //Orange 2.0 template
				$sql .= "`before_vote_template`	= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
				`css`							= '#shivs-poll-container-%POLL-ID% {\r\n    width:%POLL-WIDTH%;\r\n    background:#fff;\r\n    padding:0px;\r\n    color:#555;\r\n    overflow:hidden;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-name-%POLL-ID% {\r\n    font-weight:bold;\r\n    background:#FB6911;\r\n    color:#fff;\r\n    padding:5px;\r\n        text-align:center;\r\n        font-size:12px;\r\n}\r\n\r\n#shivs-poll-question-%POLL-ID% {\r\n    font-size:14px;\r\n    margin:5px 0px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% {  }\r\n#shivs-poll-answers-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li { \r\n    font-style:normal;\r\n    margin:0px 0px 10px 0px;\r\n    padding:0px;\r\n    font-size:12px;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li input { \r\n    margin:0px; \r\n    float:none;\r\n}\r\n#shivs-poll-answers-%POLL-ID% ul li label { \r\n    margin:0px; \r\n    font-style:normal; \r\n    font-weight:normal; \r\n    font-size:12px; \r\n    float:none;\r\n}\r\n.shivs-poll-results-%POLL-ID% {\r\n    font-size: 12px;\r\n    font-style: italic;\r\n    font-weight: normal;\r\n    margin-left: 15px;\r\n}\r\n\r\n#shivs-poll-custom-%POLL-ID% {  }\r\n#shivs-poll-custom-%POLL-ID% ul {\r\n    list-style: none outside none;\r\n    margin: 0;\r\n    padding: 0;\r\n}\r\n#shivs-poll-custom-%POLL-ID% ul li { \r\n    padding:0px;\r\n    margin:0px;    \r\n    font-size:14px;\r\n}\r\n#shivs-poll-container-%POLL-ID% input[type=\'text\'] { margin:0px 0px 5px 0px; padding:2%; width:96%; text-indent:2%; font-size:12px; }\r\n\r\n#shivs-poll-captcha-input-div-%POLL-ID% {\r\nmargin-top:5px;\r\n}\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% {\r\nwidth:30px;\r\nfloat:left;\r\nmargin-left:5px;\r\nheight:0px;\r\n}\r\n\r\n#shivs-poll-captcha-helpers-div-%POLL-ID% img {\r\nmargin-bottom:2px;\r\n}\r\n\r\n#shivs-poll-captcha-image-div-%POLL-ID% {\r\nmargin-bottom:5px;\r\n}\r\n\r\n#shivs_poll_captcha_image_%POLL-ID% {\r\nfloat:left;\r\n}\r\n\r\n.shivs_poll_clear {\r\nclear:both;\r\n}\r\n\r\n#shivs-poll-vote-%POLL-ID% {\r\n\r\n}\r\n.shivs-poll-results-bar-%POLL-ID% { background:#f5f5f5; height:10px;  }\r\n.shivs-poll-results-bar-%POLL-ID% div { background:#555; height:10px; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-vote-%POLL-ID% button { float:left; }\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% {\r\n    float: right;\r\n    margin-bottom: 20px;\r\n    margin-top: -20px;\r\n    width: auto;\r\n}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-results-%POLL-ID% a { color:#FB6911; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div#shivs-poll-back-%POLL-ID% a { color:#FB6911; text-decoration:underline; font-size:12px;}\r\n#shivs-poll-vote-%POLL-ID% div { float:left; width:100%; }\r\n\r\n#shivs-poll-container-error-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:red;\r\n    text-transform:lowercase;\r\n}\r\n\r\n#shivs-poll-container-success-%POLL-ID% {\r\n    font-size:12px;\r\n    font-style:italic;\r\n    color:green;\r\n}',
				`js`							= 'function stripBorder_%POLL-ID%(object) {\r\n    object.each(function() {\r\n            if( parseInt(jQuery(this).width() ) > 0) {\r\n                jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ")) );\r\n            }\r\n            else {\r\n                jQuery(this).css(\\" . '"' . "border-left-width\\" . '"' . ", \'0px\');\r\n                jQuery(this).css(\\" . '"' . "border-right-width\\" . '"' . ", \'0px\');\r\n            }\r\n    });\r\n}\r\nfunction stripPadding_%POLL-ID%(object) {\r\n    object.each(function() { \r\n            jQuery(this).width( parseInt( jQuery(this).width() ) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) - parseInt(jQuery(this).css(\\" . '"' . "padding-left\\" . '"' . ")) );\r\n    });\r\n}\r\n\r\nfunction strip_results_%POLL-ID%() {\r\n        stripPadding_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs_poll_li_result-%POLL-ID%\\" . '"' . ") );   \r\n    stripBorder_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-result-bar-%POLL-ID%\\" . '"' . "));\r\n}\r\n\r\njQuery(document).ready(function(e) {\r\n                if(typeof window.strip_results_%POLL-ID% == \'function\') \r\n            strip_results_%POLL-ID%();\r\n    \r\n        if(typeof window.tabulate_answers_%POLL-ID% == \'function\') \r\n            tabulate_answers_%POLL-ID%();\r\n        \r\n        if(typeof window.tabulate_results_%POLL-ID% == \'function\') \r\n            tabulate_results_%POLL-ID%();\r\n        \r\n});\r\n\r\nfunction equalWidth_%POLL-ID%(obj, cols, findWidest ) {\r\n findWidest  = typeof findWidest  !== \'undefined\' ? findWidest  : false;\r\n    if ( findWidest ) {\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                width = parseInt(thisWidth / cols); \r\n                jQuery(this).width(width);    \r\n                jQuery(this).css(\'float\', \'left\');    \r\n        });\r\n    }\r\n    else {\r\n        var widest = 0;\r\n        obj.each(function() {\r\n                var thisWidth = jQuery(this).width();\r\n                if(thisWidth > widest) {\r\n                    widest = thisWidth; \r\n                }    \r\n        });\r\n        width = parseInt( widest / cols); \r\n        obj.width(width);    \r\n        obj.css(\'float\', \'left\');    \r\n    }    \r\n}\r\n\r\nfunction tabulate_answers_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID%\\" . '"' . "), %ANSWERS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-answer-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %ANSWERS-TABULATED-COLS%, true );\r\n}\r\n\r\nfunction tabulate_results_%POLL-ID%() {\r\n    equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID%\\" . '"' . "), %RESULTS-TABULATED-COLS% );\r\n        //equalWidth_%POLL-ID%( jQuery(\\" . '"' . "#shivs-poll-container-%POLL-ID% .shivs-poll-li-result-%POLL-ID% .shivs-poll-results-bar-%POLL-ID% div \\" . '"' . "), %RESULTS-TABULATED-COLS%, true );\r\n}',
				`last_modified`					= '" . current_time( 'mysql' ) . "' ";
			break;

			case '15': //Orange 3.0 template
				$sql .= "`before_vote_template`	= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-ANSWER-CHECK-INPUT% \r\n            %POLL-ANSWER-LABEL%\r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_CONTAINER]\r\n        [OTHER_ANSWER_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-answer-%POLL-ID%\\" . '"' . ">\r\n            %POLL-OTHER-ANSWER-CHECK-INPUT% \r\n            %POLL-OTHER-ANSWER-LABEL% \r\n            <span class=\\" . '"' . "shivs-poll-results-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-RESULT-LABEL%</span>\r\n            %POLL-OTHER-ANSWER-TEXT-INPUT% \r\n            %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/OTHER_ANSWER_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-custom-%POLL-ID%\\" . '"' . ">\r\n    <ul>\r\n        [CUSTOM_FIELD_CONTAINER]\r\n        <li>%POLL-CUSTOM-FIELD-LABEL% %POLL-CUSTOM-FIELD-TEXT-INPUT%</li>\r\n        [/CUSTOM_FIELD_CONTAINER]\r\n    </ul>\r\n</div>    \r\n[CAPTCHA_CONTAINER]\r\n<div id=\"shivs-poll-captcha-%POLL-ID%\">\r\n    <div class=\"shivs-poll-captcha-image-div\" id=\"shivs-poll-captcha-image-div-%POLL-ID%\">\r\n        %CAPTCHA-IMAGE%\r\n        <div class=\"shivs-poll-captcha-helpers-div\" id=\"shivs-poll-captcha-helpers-div-%POLL-ID%\">%RELOAD-CAPTCHA-IMAGE% </div>\r\n        <div class=\"shivs_poll_clear\"></div>\r\n    </div>\r\n    %CAPTCHA-LABEL%\r\n    <div class=\"shivs-poll-captcha-input-div\" id=\"shivs-poll-captcha-input-div-%POLL-ID%\">%CAPTCHA-INPUT%</div>\r\n</div>\r\n[/CAPTCHA_CONTAINER]\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-VOTE-BUTTON%</div>\r\n    <div id=\\" . '"' . "shivs-poll-results-%POLL-ID%\\" . '"' . ">%POLL-VIEW-RESULT-LINK%</div>\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n</div>',
				`after_vote_template`			= '<div id=\\" . '"' . "shivs-poll-name-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-name\\" . '"' . ">%POLL-NAME%</div>\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label-%POLL-ID%\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text-%POLL-ID%\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result-%POLL-ID%\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per-%POLL-ID%\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <div>\r\n             %POLL-ANSWER-RESULT-BAR%\r\n           </div>\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n<div id=\\" . '"' . "shivs-poll-vote-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-footer\\" . '"' . ">\r\n    <div>%POLL-TOTAL-ANSWERS%</div>\r\n    <div>%POLL-TOTAL-VOTES%</div>\r\n    <div id=\\" . '"' . "shivs-poll-back-%POLL-ID%\\" . '"' . ">%POLL-BACK-TO-VOTE-LINK%</div>\r\n</div>',
				`before_start_date_template`	= 'This poll is about to <br>\r\nstart at %POLL-START-DATE%<br>\r\nand finish at %POLL-END-DATE%<br>',
				`after_end_date_template`		= 'This poll is closed!\r\nPoll activity: <br>\r\nstart_date %POLL-START-DATE%<br>\r\nend_date %POLL-END-DATE%<br>\r\n\r\nPoll Results:\r\n<div id=\\" . '"' . "shivs-poll-question-%POLL-ID%\\" . '"' . " class=\\" . '"' . "shivs-poll-question\\" . '"' . ">%POLL-QUESTION%</div>\r\n<div id=\\" . '"' . "shivs-poll-answers-1\\" . '"' . " class=\\" . '"' . "shivs-poll-answers\\" . '"' . ">\r\n    <ul>\r\n        [ANSWER_RESULT_CONTAINER]\r\n        <li class=\\" . '"' . "shivs-poll-li-result-%POLL-ID%\\" . '"' . ">\r\n            <label class=\\" . '"' . "pds-feedback-label\\" . '"' . ">\r\n                <span class=\\" . '"' . "pds-answer-text\\" . '"' . ">%POLL-ANSWER-LABEL%</span>\r\n                <span class=\\" . '"' . "pds-feedback-result\\" . '"' . ">\r\n                    <span class=\\" . '"' . "pds-feedback-per\\" . '"' . "> %POLL-ANSWER-RESULT-LABEL%</span>\r\n                </span>\r\n            </label>\r\n            <span class=\\" . '"' . "pds-clear\\" . '"' . " style=\\" . '"' . "display: block;clear: both;height:1px;line-height:1px;\\" . '"' . "> </span>\r\n    %POLL-ANSWER-RESULT-BAR%\r\n        </li>\r\n        [/ANSWER_RESULT_CONTAINER]\r\n    </ul>\r\n</div>\r\n%POLL-VOTE-BUTTON%',
	...
	public function make_poll_from_request_data( $request = array(), $config = NULL ) {
		...
		$this->poll['id']         = isset( $request['shivs_poll_id'] ) ? trim( $request['shivs_poll_id'] ) : NULL;

		$this->poll['name']       = isset( $request['shivs_poll_name'] ) ? shivs_poll_kses( trim( $request['shivs_poll_name'] ) ) : NULL;

		$this->poll['question']   = isset( $request['shivs_poll_question'] ) ? shivs_poll_kses( trim( $request['shivs_poll_question'] ) ) : NULL;

		$this->poll['start_date'] = isset( $request['shivs_poll_options']['start_date'] ) ? trim( $request['shivs_poll_options']['start_date'] ) : NULL;

		if ( !isset( $request['shivs_poll_options']['never_expire'] ) ){
			$this->poll['end_date'] = isset( $request['shivs_poll_options']['end_date'] ) ? trim( $request['shivs_poll_options']['end_date'] ) : NULL;
		}
		else {
			$this->poll['end_date'] = '9999-12-31 23:59:59';
		}

		$this->poll['show_in_archive'] = isset( $request['shivs_poll_options']['show_in_archive'] ) ? trim( $request['shivs_poll_options']['show_in_archive'] ) : NULL;

		$this->poll['archive_order']   = isset( $request['shivs_poll_options']['archive_order'] ) ? trim( $request['shivs_poll_options']['archive_order'] ) : NULL;
	}
	...
	public function make_answers_from_request_data( $request = array(), $config = NULL ) {
		...
		if ( isset ( $request['shivs_poll_answer'] ) ) {
			if ( count( $request['shivs_poll_answer'] ) > 0 ) {
				foreach ( $request['shivs_poll_answer'] as $answer_id => $answer_value ) {
					$answer['answer'] = $answer_value;
					$answer['id']     = isset( $request['shivs_poll_answer_ids'][$answer_id] ) ? $request['shivs_poll_answer_ids'][$answer_id] : NULL;
		...
		if ( isset ( $request['shivs_poll_options']['allow_other_answers'] ) ) {
			if ( 'yes' == $request['shivs_poll_options']['allow_other_answers'] ) {
				$answer['answer'] = isset( $request['shivs_poll_options']['other_answers_label'] ) ? $request['shivs_poll_options']['other_answers_label'] : 'Other';
		...
		if ( isset( $request['shivs_poll_customfield'] ) ) {
			if ( count( $request['shivs_poll_customfield'] ) > 0 ) {
				...
				foreach ( $request['shivs_poll_customfield'] as $customfield_id => $customfield_value ) {
					...
					$custom_field['id']           = isset( $request['shivs_poll_customfield_ids'][$customfield_id] ) ? $request['shivs_poll_customfield_ids'][$customfield_id] : NULL;
					$custom_field['required']     = isset( $request['shivs_poll_customfield_required'][$customfield_id] ) ? 'yes' : 'no';
	...
	public function add_poll_to_database( $request = array(), $config = NULL ) {
		...
		if ( $this->verify_request_data( $request, $config ) ) {
			...
			if ( !isset( $result['id'] ) ) {
				...
				// push polls to database
				$this->poll['id'] = self::insert_poll_to_database( $this->poll );

				if ( isset( $request['shivs_poll_options']['auto_generate_poll_page'] ) ) {
					if ( 'yes' == $request['shivs_poll_options']['auto_generate_poll_page'] ) {
						$_p                   = array();
						$_p['post_title']     = $this->poll['name'];
						$_p['post_content']   = "[shivs_poll id='" . $this->poll['id'] . "']";
						$_p['post_status']    = 'publish';
						$_p['post_type']      = 'page';
						$_p['comment_status'] = 'open';
						$_p['ping_status']    = 'open';
						$_p['post_category']  = array( 1 ); // default category

						$poll_page_id = wp_insert_post( $_p );

						$request['shivs_poll_options']['poll_page_url']               = get_permalink( $poll_page_id );
						$request['shivs_poll_options']['has_auto_generate_poll_page'] = 'yes';
					}
				}
				...
				if ( isset( $request['shivs_poll_options'] ) ){
					foreach ( $request['shivs_poll_options'] as $option_name => $option_value ) {
						if ( $default_options ){
				...
				if ( isset( $request['shivs_poll_options']['schedule_reset_poll_date'] ) ){
					$poll_options['schedule_reset_poll_date'] = strtotime( $request['shivs_poll_options']['schedule_reset_poll_date'] );
				}
				else {
					$poll_options['schedule_reset_poll_date'] = current_time( 'timestamp' );
				}

				if ( 'yes' == $request['shivs_poll_options']['schedule_reset_poll_stats'] ){
					$default_options['start_scheduler'] = 'yes';
				}
				else {
					$change_start_scheduler_to_no = true;
					$shivs_polls                    = self::get_shivs_polls_fields( array( 'id' ) );
					if ( count( $shivs_polls ) > 0 ){
						foreach ( $shivs_polls as $shivs_poll_id ) {
							if ( $shivs_poll_id['id'] != $this->poll['id'] ){
								$shivs_poll_options = get_shivs_poll_meta( $shivs_poll_id['id'], 'options', true );
								if ( isset( $shivs_poll_options['schedule_reset_poll_stats'] ) && 'yes' == $shivs_poll_options['schedule_reset_poll_stats'] ){
				...
				// if poll option is greater than 0
				update_shivs_poll_meta( $this->poll['id'], 'options', $poll_options );
				...
				if ( count( $this->answers ) > 0 ){
					foreach ( $this->answers as $answer ) {
						...
						// push poll answer options
						if ( 'other' != $answer['type'] ){
							if ( isset( $request['shivs_poll_answer_options'][$answer['name']] ) ){
								$poll_answer_options = array();
								foreach ( $request['shivs_poll_answer_options'][$answer['name']] as $option_name => $option_value ) {
								...
								// if poll count is greater than 0
								update_shivs_poll_answer_meta( $answer_id, 'options', $poll_answer_options, false );
							}
						}
					}
				}

				// push custom fields to database
				...
				$this->error = __( 'This poll already exists! Please choose another name!', 'shivs_poll' );
				return false;
			}
		}
		else {
	...
	public function edit_poll_in_database( $request = array(), $config = NULL ) {
		...
		if ( $this->verify_request_data( $request, $config ) ) {
			...
			if ( isset( $result['id'] ) ){
				...
				// update polls to database
				self::update_poll_in_database( $this->poll );

				$poll_old_options = get_shivs_poll_meta( $this->poll['id'], 'options', true );

				if ( !isset( $poll_old_options['has_auto_generate_poll_page'] ) ){
					$poll_old_options['has_auto_generate_poll_page'] = 'no';
				}
				if ( !isset( $request['shivs_poll_options']['auto_generate_poll_page'] ) ){
					$request['shivs_poll_options']['auto_generate_poll_page'] = 'no';
				}
					...
					if ( count( $all_poll_answers ) > 0 ){
						foreach ( $all_poll_answers as $answer ) {
							if ( !in_array( $answer['id'], $answer_ids_for_not_remove ) ){
								...
								delete_shivs_poll_answer_meta( $answer['id'], 'options' );
							}
						}
					}
				}
				...
				$this->error = __( 'This poll doesn`t exist!', 'shivs_poll' );
				return false;
			}
		}
		else {
	...
	public static function get_poll_answers( $poll_id, $types = array( 'default' ), $order = 'id', $order_dir = '', $include_others = false, $percentages_decimals = 0 ) {
		...
		if ( $is_votes_sort ) {
			...
			usort( $answers, array( 'Shivs_Poll_Model', "sort_answers_by_votes_" . $order_dir . "_callback" ) );
		}
		if ( $include_others ) {
			if ( 'answer' == $order ) {
				$order_dir = ( '' == $order_dir ) ? 'asc' : $order_dir;
				usort( $answers, array( 'Shivs_Poll_Model', "sort_answers_alphabetical_" . $order_dir . "_callback" ) );
			}
	...
	private static function get_poll_answer_by_field( $poll_id, $field_name, $field_value, $field_type = '%s' ) {
		$answer = $GLOBALS['wpdb']->get_row( $GLOBALS['wpdb']->prepare( "
				SELECT *
				FROM {$GLOBALS['wpdb']->shivs_poll_answers}
	...
	public static function get_poll_customfields( $poll_id ) {
		...
		$result = $wpdb->get_results( $wpdb->prepare( "
				SELECT *
				FROM " . $wpdb->shivs_poll_custom_fields . "
				WHERE poll_id = %d
				", $poll_id ), ARRAY_A );
		return $result;
	}
	...
	public static function get_poll_customfields_logs( $poll_id, $orderby = 'vote_date', $order = 'desc', $offset = 0, $per_page = 99999999, $sdate = '', $edate = '' ) {
		...
		$result = $wpdb->get_results( $wpdb->prepare( "
				...
				FROM " . $wpdb->shivs_poll_votes_custom_fields . "
	...
	public static function get_default_template() {
		...
		$result = $wpdb->get_row( "
			SELECT *
			FROM " . $wpdb->shivs_poll_templates . "
	...
	private static function insert_answer_to_database( $answer ) {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "
				INSERT INTO " . $wpdb->shivs_poll_answers . "
				SET poll_id = %d,
				answer = %s,
				votes = %d,
				status = %s,
				type = %s
				",
				$answer['poll_id'], shivs_poll_kses( $answer['answer'] ), $answer['votes'], $answer['status'], $answer['type'] ) );
		return $wpdb->insert_id;
	}
	...
	private static function update_answer_in_database( $answer ) {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "
				UPDATE " . $wpdb->shivs_poll_answers . "
				SET answer = %s
				WHERE id = %d
				", shivs_poll_kses( $answer['answer'] ), $answer['id'] ) );
	}
	...
	public static function get_shivs_polls_filter_search( $orderby = 'id', $order = 'desc', $filter = array( 'field' => NULL, 'value' => NULL, 'operator' => '=' ), $search = array( 'fields' => array(), 'value' => NULL ) ) {
		global $wpdb;
		$sql        = "SELECT * FROM " . $wpdb->shivs_polls;
		$sql_filter = '';
		$sql_search = '';
	...
	public static function get_shivs_polls_fields( $fields = array( 'id' ) ) {
		global $wpdb;
		$fields_text = '';
		if ( count( $fields ) > 0 ){
		...
		$sql = 'SELECT ' . $fields_text . ' FROM ' . $wpdb->shivs_polls;
		return $wpdb->get_results( $sql, ARRAY_A );
	}
	...
	public static function get_logs_filter_search( $orderby = 'id', $order = 'desc', $search = array( 'fields' => array(), 'value' => NULL ), $poll_id = NULL, $offset = 0, $per_page = 99999999, $sdate = '', $edate = '' ) {
		...
		if ( 'id' == $orderby ){
			$orderby = $wpdb->shivs_poll_logs . ".id";
		}
		...
		FROM (
		SELECT
		" . $wpdb->shivs_poll_logs . ".id,
		" . $wpdb->shivs_poll_logs . ".vote_id,
		" . $wpdb->shivs_poll_logs . ".poll_id,
		" . $wpdb->shivs_poll_logs . ".ip,
		" . $wpdb->shivs_poll_logs . ".http_referer,
		" . $wpdb->shivs_poll_logs . ".tr_id,
		" . $wpdb->shivs_poll_logs . ".vote_date,
		" . $wpdb->shivs_poll_logs . ".other_answer_value,
		" . $wpdb->shivs_polls . ".name,
		IF (" . $wpdb->shivs_poll_answers . ".answer = 'Other' , concat( 'Other - ', " . $wpdb->shivs_poll_logs . ".other_answer_value ), " . $wpdb->shivs_poll_answers . ".answer ) as answer,
		IF( " . $wpdb->shivs_poll_logs . ".user_type = 'facebook', " . $wpdb->shivs_poll_facebook_users . ".name ,IFNULL( " . $wpdb->users . ".display_name, '" . __( '_Anonymous', 'shivs_poll' ) . "' )  ) as user_nicename,
		IF( " . $wpdb->shivs_poll_logs . ".user_type = 'facebook', " . $wpdb->shivs_poll_facebook_users . ".email , " . $wpdb->users . ".user_email ) as user_email,
		IF( " . $wpdb->shivs_poll_logs . ".user_type = 'facebook', 'facebook' , 'wordpress' ) as user_from,
		" . $wpdb->shivs_poll_logs . ".user_id

		FROM " . $wpdb->shivs_poll_logs . "
		LEFT JOIN (" . $wpdb->shivs_polls . ',' . $wpdb->shivs_poll_answers . ")
		ON (
		" . $wpdb->shivs_poll_logs . ".poll_id = " . $wpdb->shivs_polls . ".id AND
		" . $wpdb->shivs_poll_logs . ".answer_id = " . $wpdb->shivs_poll_answers . ".id
		)
		LEFT JOIN (" . $wpdb->users . ")
		ON (
		" . $wpdb->shivs_poll_logs . ".user_id = " . $wpdb->users . ".ID
		)
		LEFT JOIN (" . $wpdb->shivs_poll_facebook_users . ")
		ON (
		" . $wpdb->shivs_poll_logs . ".user_id = " . $wpdb->shivs_poll_facebook_users . ".id
		)
		";
		$sql .= ' ) as logs  ';
		$sql .= $sql_search;
	...
	public static function get_total_logs_filter_search( $search = array( 'fields' => array(), 'value' => NULL ), $poll_id = NULL, $sdate = '', $edate = '' ) {
		...
		" . $wpdb->shivs_poll_logs . ".vote_date,
		" . $wpdb->shivs_poll_logs . ".other_answer_value,
		" . $wpdb->shivs_poll_logs . ".user_type,
		" . $wpdb->shivs_polls . ".name,
		IF (" . $wpdb->shivs_poll_answers . ".answer = 'Other' , concat( 'Other - ', " . $wpdb->shivs_poll_logs . ".other_answer_value ), " . $wpdb->shivs_poll_answers . ".answer ) as answer,
		IF( " . $wpdb->shivs_poll_logs . ".user_type = 'facebook', " . $wpdb->shivs_poll_facebook_users . ".name ,IFNULL( " . $wpdb->users . ".display_name, '" . __( 'Anonymous', 'shivs_poll' ) . "' )  ) as user_nicename,
		IF( " . $wpdb->shivs_poll_logs . ".user_type = 'facebook', " . $wpdb->shivs_poll_facebook_users . ".email , " . $wpdb->users . ".user_email ) as user_email,
		IF( " . $wpdb->shivs_poll_logs . ".user_type = 'facebook', 'facebook' , 'wordpress' ) as user_from,
		" . $wpdb->shivs_poll_logs . ".user_id
	...
	public static function get_bans_filter_search( $orderby = 'id', $order = 'desc', $search = array( 'fields' => array(), 'value' => NULL ), $type = NULL, $poll_id = NULL, $offset = 0, $per_page = 99999999 ) {
		...
		if ( 'id' == $orderby ){
			$orderby = $wpdb->shivs_poll_bans . ".id";
		}

		$sql_search = ' ';

		if ( $poll_id ){
			$sql_search .= $wpdb->prepare( 'WHERE ' . $wpdb->shivs_poll_bans . '.poll_id = %d', $poll_id );
		}
		if ( $type ){
			if ( $poll_id ){
				$sql_search .= ' AND  ';
			...
			$sql_search .= $wpdb->prepare( $wpdb->shivs_poll_bans . '.type= %s', $type );
		}
		if ( count( $search['fields'] ) > 0 ){
			if ( $poll_id || $type ){
				$sql_search .= ' AND ( ';
		...
		$sql = "SELECT
		" . $wpdb->shivs_poll_bans . ".id,
		" . $wpdb->shivs_poll_bans . ".value,
		" . $wpdb->shivs_poll_bans . ".type,
		IFNULL( " . $wpdb->shivs_polls . ".name, '" . __( 'All Shivs Polls', 'shivs_poll' ) . "' ) as name

		FROM " . $wpdb->shivs_poll_bans . "
		LEFT JOIN (" . $wpdb->shivs_polls . ")
		ON (
		" . $wpdb->shivs_poll_bans . ".poll_id = " . $wpdb->shivs_polls . ".id
		)
		";
		$sql .= $sql_search;
		$sql .= ' ORDER BY ' . esc_attr( $orderby ) . ' ' . esc_attr( $order );
	...
	private static function delete_poll_answers_from_db( $answer_id, $poll_id ) {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "
				DELETE FROM " . $wpdb->shivs_poll_answers . "
				WHERE id = %d AND
				poll_id = %d
				", $answer_id, $poll_id ) );
	}
	...
	private static function insert_custom_field_to_database( $custom_field ) {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "
				INSERT INTO " . $wpdb->shivs_poll_custom_fields . "
				SET poll_id = %d,
				custom_field = %s,
				required = %s,
				status = %s
	...
	private static function update_custom_field_in_database( $custom_field ) {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "
				UPDATE " . $wpdb->shivs_poll_custom_fields . "
				SET custom_field = %s,
				required = %s
				WHERE id = %d
	...
	private static function delete_poll_customfields_from_db( $customfield_id, $poll_id ) {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "
				DELETE FROM " . $wpdb->shivs_poll_custom_fields . "
				WHERE id = %d AND
				poll_id = %d
	...
	public static function delete_poll_from_db( $poll_id ) {
		...
		$wpdb->query( $wpdb->prepare( '
				DELETE FROM ' . $wpdb->shivs_polls . '
				WHERE id = %d
				', $poll_id ) );

		delete_shivs_poll_meta( $poll_id, 'options' );
	...
	public static function clone_poll( $poll_id ) {
		...
		if ( $poll_details ){
			...
			update_shivs_poll_meta( $new_poll_id, 'options', $poll_options[0] );
			...
			if ( $answers ){
				foreach ( $answers as $answer ) {
					...
					if ( $answer['type'] != 'other' ){
						$answer_options = get_shivs_poll_answer_meta( $answer['id'], 'options' );
						update_shivs_poll_answer_meta( $new_answer_id, 'options', $answer_options[0] );
					}
				}
			}
	...
	public static function get_shivs_poll_templates_search( $orderby = 'last_modified', $order = 'desc', $search = array( 'fields' => array(), 'value' => NULL ) ) {
		global $wpdb;
		$sql        = "SELECT * FROM " . $wpdb->shivs_poll_templates;
		$sql_search = '';
		if ( count( $search['fields'] ) > 0 ){
			$sql_search .= ' ( ';
			foreach ( $search['fields'] as $field ) {
	...
	private static function insert_poll_template_to_database( $template ) {
		...
		wp_get_current_user();
		$wpdb->query( $wpdb->prepare( "
				INSERT INTO " . $wpdb->shivs_poll_templates . "
				SET
				template_author = %d,
				name = %s,
	...
	private static function insert_ban_to_database( $ban ) {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "
				INSERT INTO " . $wpdb->shivs_poll_bans . "
				SET poll_id = %d,
				type = %s,
				value = %s
				", $ban['poll_id'], $ban['type'], $ban['value'] ) );
	...
	public static function delete_poll_template_from_db( $template_id ) {
		...
		$wpdb->query( $wpdb->prepare( '
				DELETE FROM ' . $wpdb->shivs_poll_templates . '
				WHERE id = %d
				', $template_id ) );
	}
	...
	public static function delete_poll_log_from_db( $log_id ) {
		...
		$wpdb->query( $wpdb->prepare( '
				DELETE FROM ' . $wpdb->shivs_poll_logs . '
				WHERE id = %d
				', $log_id ) );
	}
	...
	public static function delete_poll_ban_from_db( $ban_id ) {
		...
		$wpdb->query( $wpdb->prepare( '
				DELETE FROM ' . $wpdb->shivs_poll_bans . '
				WHERE id = %d
				', $ban_id ) );
	}
	...
	public static function reset_votes_for_poll( $poll_id ) {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( 'UPDATE ' . $wpdb->shivs_polls . ' SET total_votes = 0, total_answers = 0 WHERE id = %d', $poll_id ) );
		$wpdb->query( $wpdb->prepare( 'UPDATE ' . $wpdb->shivs_poll_answers . ' SET votes = 0 WHERE poll_id = %d', $poll_id ) );
		$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . $wpdb->shivs_poll_voters . ' WHERE poll_id = %d', $poll_id ) );
	}
	...
	private static function insert_vote_in_database( $answer = array() ) {
		...
		$wpdb->query( $wpdb->prepare( "
				INSERT INTO " . $wpdb->shivs_poll_logs . "
				SET
				poll_id                = %d,
				vote_id                = %s,
				answer_id            = %d,
	...
	private static function insert_voter_in_database( $voter = array() ) {
		...
		$wpdb->query( $wpdb->prepare( "
				INSERT INTO " . $wpdb->shivs_poll_voters . "
				SET
				poll_id                = %d,
				user_id                = %s,
				user_type            = %s
	...
	public static function delete_all_poll_logs( $poll_id ) {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "DELETE FROM " . $wpdb->shivs_poll_logs . " WHERE poll_id = %d ", $poll_id ) );
	}
	...
	private static function insert_vote_custom_field_in_database( $custom_field = array() ) {
		...
		$wpdb->query( $wpdb->prepare( "
				INSERT INTO " . $wpdb->shivs_poll_votes_custom_fields . "
				SET
				poll_id                = %d,
				vote_id                = %s,
				custom_field_id        = %d,
	...
	private static function insert_facebook_user_in_database( $user_details = array() ) {
		...
		$user_exist = $wpdb->get_row( $wpdb->prepare( "
				SELECT * FROM " . $wpdb->shivs_poll_facebook_users . "
				WHERE
				fb_id    = %s
				LIMIT 0,1
		...
		if ( $user_exist ){
			...
			$wpdb->query( $wpdb->prepare( "
					UPDATE " . $wpdb->shivs_poll_facebook_users . "
					SET
					name                = %s,
					first_name            = %s,
					last_name            = %s,
			...
			$wpdb->query( $wpdb->prepare( "
					INSERT INTO " . $wpdb->shivs_poll_facebook_users . "
					SET
					fb_id                = %s,
					name                = %s,
					first_name            = %s,
	...
	public function register_vote( $request ) {
		...
		if ( wp_verify_nonce( $request['shivs-poll-nonce-' . $poll_id . $unique_id], 'shivs_poll-' . $poll_id . $unique_id . '-user-actions' ) ){
			$poll_details = $this->poll;
			$poll_options = $this->poll_options;
			$vote_id      = uniqid( 'vote_id_' );
			$vote_type    = $request['vote_type'];
			$tr_id        = $request['shivs_poll_tr_id'];
			...
			if ( 'facebook' == $vote_type ){
				if ( !is_array( $facebook_user_details ) ){
					$this->error = __( 'An Error Has Occured!', 'shivs_poll' );
					return false;
				}
				...
				if ( $facebook_error == 'access_denied' ){
					$this->error = __( 'You Don`t Have Permission to Vote! You must authorize SHIVS POLL application in your facebook account!', 'shivs_poll' );
					return false;
				}
				if ( $facebook_error == 'access_error' ){
					$this->error = __( 'You Don`t Have Permission to Vote!', 'shivs_poll' );
					return false;
				}
			...
			$current_date = SHIVS_POLL_MODEL::get_mysql_curent_date();
				...
				if ( $current_date >= $poll_details['start_date'] ){
					if ( $current_date <= $poll_details['end_date'] ){
						if ( 'closed' == $poll_details['status'] ){
							$this->error = __( 'This poll is closed!', 'shivs_poll' );
							return false;
						}
						else {
								...
								if ( $this->user_have_votes_to_vote( $voter ) ){
									if ( isset ( $request['shivs_poll_answer'] ) ){
										if ( 'yes' == $poll_options['allow_multiple_answers'] ){
											if ( count( $request['shivs_poll_answer'] ) <= intval( $poll_options['allow_multiple_answers_number'] ) ){
												if ( count( $request['shivs_poll_answer'] ) >= intval( $poll_options['allow_multiple_answers_min_number'] ) ){
													foreach ( $request['shivs_poll_answer'] as $answer_id ) {
														$answer              = array();
														...
														if ( 'other' == $answer_details['type'] ){
															if ( isset( $request['shivs_poll_other_answer'] ) ){
																if ( '' != strip_tags( trim( $request['shivs_poll_other_answer'] ) ) ){
																	$answer['other_answer_value'] = strip_tags( $request['shivs_poll_other_answer'] );
																	$answer['type']               = 'other';
																}
																else {
																	$this->error = __( 'Your other answer is empty!', 'shivs_poll' );
																	return false;
																}
															}
															else {
																$this->error = __( 'Your other answer is invalid!', 'shivs_poll' );
																return false;
															}
														}
														$answers[] = $answer;
													...
													$this->error = __( 'Too few answers! Only more than ', 'shivs_poll' ) . $poll_options['allow_multiple_answers_min_number'] . __( ' answers allowed!', 'shivs_poll' );
													return false;
												}
											}
											else {
												$this->error = __( 'Too many answers! Only ', 'shivs_poll' ) . $poll_options['allow_multiple_answers_number'] . __( ' answers allowed!', 'shivs_poll' );
												return false;
											}
										}
										else {
											...
											$answer['poll_id']   = $poll_id;
											$answer['vote_id']   = $vote_id;
											$answer['answer_id'] = $request['shivs_poll_answer'];
											$answer['ip']        = self::get_ip();
											$answer['user_id']   = $current_user->ID;
											$answer['type']      = 'default';
											...
											$answer['host']               = esc_attr( @gethostbyaddr( self::get_ip() ) );
											$answer['other_answer_value'] = '';
											$answer_details               = self::get_poll_answer_by_id( $request['shivs_poll_answer'] );
											if ( 'other' == $answer_details['type'] ){
												if ( isset( $request['shivs_poll_other_answer'] ) ){
													if ( '' != strip_tags( trim( $request['shivs_poll_other_answer'] ) ) ){
														$answer['other_answer_value'] = strip_tags( $request['shivs_poll_other_answer'] );
														...
														$this->error = __( 'Your other answer is empty or contains invalid tags!', 'shivs_poll' );
														return false;
													}
												}
												else {
										...
										if ( count( $answers ) > 0 ){
											...
											if ( count( $poll_custom_fields ) > 0 ){
												if ( isset( $request['shivs_poll_customfield'] ) ){
													foreach ( $poll_custom_fields as $custom_field ) {
														if ( isset( $request['shivs_poll_customfield'][$custom_field['id']] ) ){
															if ( '' == trim( $request['shivs_poll_customfield'][$custom_field['id']] ) && 'yes' == $custom_field['required'] ){
																$this->error = __( 'Custom field ', 'shivs_poll' ) . $custom_field['custom_field'] . __( ' is required! ', 'shivs_poll' );
																return false;
															}
															else {
																if ( $request['shivs_poll_customfield'][$custom_field['id']] != '' ){
																	...
																	$new_custom_field['custom_field_value'] = strip_tags( trim( $request['shivs_poll_customfield'][$custom_field['id']] ) );
																	$custom_fields[]                        = $new_custom_field;
																}
															}
														}
														else {
															$this->error = __( 'Custom field ', 'shivs_poll' ) . $custom_field['custom_field'] . __( ' is missing! ', 'shivs_poll' );
															return false;
														}
													}
												}
												else {
													$this->error = __( 'Custom fields are missing!', 'shivs_poll' );
													return false;
												}
											}

											if ( 'yes' == $poll_options['use_captcha'] ){
												require_once( SHIVS_POLL_INC . '/securimage.php' );
												$img            = new Shivs_Poll_Securimage();
												$img->namespace = 'shivs_poll_' . $poll_id . $unique_id;
												if ( $img->check( $_REQUEST['shivs_poll_captcha_input'][$poll_id] ) ){
													$cookie_ids                 = '';
													$votes                      = 0;
													...
													if ( 'yes' == $poll_options['send_email_notifications'] ){
														...
														add_filter( 'wp_mail_content_type', 'shivs_poll_set_html_content_type' );
														$is_sent = wp_mail( $poll_options['email_notifications_recipients'], $subject, $body, $headers );
														remove_filter( 'wp_mail_content_type', 'shivs_poll_set_html_content_type' );
													}
													return do_shortcode( $this->return_poll_html( array( 'tr_id' => $tr_id, 'location' => $location ) ) );
												}
												else {
													$this->error = __( 'Incorrect security code entered!', 'shivs_poll' );
													return false;
												}
											}
											else {
											...
											$this->error = __( 'No vote was registered!', 'shivs_poll' );
											return false;
										}
									}
									else {
										$this->error = __( 'No answer selected!', 'shivs_poll' );
										return false;
									}
								}
								else {
									$this->error = __( 'You have run out of votes!', 'shivs_poll' );
									return false;
								}
							}
							else {
								$this->error = __( 'You Already voted!', 'shivs_poll' );
								return false;
							}
						}
					}
					...
					$this->error = __( 'You can vote once the poll starts!', 'shivs_poll' );
					return false;
				}
			}
			else {
				$this->error = __( 'You are not allowed to vote!', 'shivs_poll' );
				return false;
			}
		}
		else {
			$this->error = __( 'Bad Request!', 'shivs_poll' );
			return false;
		}
	}
	...
	public function return_template_preview_html( $template_id = '', $loc = 1 ) {
		if ( '' == $template_id ){
			...
			$template = str_ireplace( '%POLL-VOTE-BUTTON%', '<button class="shivs_poll_vote_button" >Vote</button>', $template );
			...
			foreach ( $answers as $key => $answer ) {
				$temps = str_ireplace( '%POLL-ANSWER-CHECK-INPUT%', '<input type="radio" value="' . $answer . '" name="shivs_poll_answer-' . $loc . '" id="shivs-poll-answer-' . $loc . '-' . $key . '" />', $m );
				$temps = str_ireplace( '%POLL-ANSWER-LABEL%', '<label for="shivs-poll-answer-' . $loc . '-' . $key . '">' . $answer . '</label>', $temps );
				$ts .= $temps;
			}
			...
			$t .= '<div id="shivs-poll-container-preview-' . $loc . '" class="shivs-poll-container">';
			$t .= '' . $template . '</div>';
			return $t;
		}
	}
	...
	public function return_poll_html( $attr = array( 'tr_id' => '', 'location' => 'page', 'load_css' => false, 'load_js' => false ) ) {
		...
		if ( function_exists( 'icl_translate' ) ){
			$poll_details['question'] = icl_translate( 'shivs_poll', $poll_details['id'] . '_question', $poll_details['question'] );
			$poll_details['name']     = icl_translate( 'shivs_poll', $poll_details['id'] . '_poll_title', $poll_details['name'] );

			//$poll_options['other_answers_label'] = icl_translate( 'shivs_poll', $poll_details['id'] .'_other_answers_label', $poll_options['other_answers_label'] );
			$poll_options['singular_answer_result_votes_number_label'] = icl_translate( 'shivs_poll', $poll_options['id'] . '_singular_answer_result_votes_number_label', $poll_options['singular_answer_result_votes_number_label'] );
			$poll_options['plural_answer_result_votes_number_label']   = icl_translate( 'shivs_poll', $poll_options['id'] . '_plural_answer_result_votes_number_label', $poll_options['plural_answer_result_votes_number_label'] );
			$poll_options['vote_button_label']                         = icl_translate( 'shivs_poll', $poll_details['id'] . '_vote_button_label', $poll_options['vote_button_label'] );
			$poll_options['view_results_link_label']                   = icl_translate( 'shivs_poll', $poll_details['id'] . '_view_results_link_label', $poll_options['view_results_link_label'] );
			$poll_options['view_back_to_vote_link_label']              = icl_translate( 'shivs_poll', $poll_details['id'] . '_view_back_to_vote_link_label', $poll_options['view_back_to_vote_link_label'] );
			$poll_options['view_total_votes_label']                    = icl_translate( 'shivs_poll', $poll_details['id'] . '_view_total_votes_label', $poll_options['view_total_votes_label'] );
			$poll_options['view_total_answers_label']                  = icl_translate( 'shivs_poll', $poll_details['id'] . '_view_total_answers_label', $poll_options['view_total_answers_label'] );
			$poll_options['view_total_voters_label']                   = icl_translate( 'shivs_poll', $poll_details['id'] . '_view_total_voters_label', $poll_options['view_total_voters_label'] );
			$poll_options['view_poll_archive_link_label']              = icl_translate( 'shivs_poll', $poll_details['id'] . '_view_poll_archive_link_label', $poll_options['view_poll_archive_link_label'] );
			$poll_options['answer_result_label']                       = icl_translate( 'shivs_poll', $poll_details['id'] . '_answer_result_label', $poll_options['answer_result_label'] );
			$poll_options['vote_permisions_facebook_label']            = icl_translate( 'shivs_poll', $poll_details['id'] . '_vote_permisions_facebook_label', $poll_options['vote_permisions_facebook_label'] );
			$poll_options['vote_permisions_wordpress_label']           = icl_translate( 'shivs_poll', $poll_details['id'] . '_vote_permisions_wordpress_label', $poll_options['vote_permisions_wordpress_label'] );
			$poll_options['vote_permisions_anonymous_label']           = icl_translate( 'shivs_poll', $poll_details['id'] . '_vote_permisions_anonymous_label', $poll_options['vote_permisions_anonymous_label'] );
		}
		...
		if ( $current_date >= $poll_details['start_date'] ){
			if ( $current_date <= $poll_details['end_date'] ){
				if ( !$is_voted ){
					...
					$template = str_ireplace( '%POLL-VOTE-BUTTON%', '<button class="shivs_poll_vote_button" id="shivs_poll_vote-button-' . $poll_id . $unique_id . '" onclick="shivs_poll_register_vote(\'' . $poll_id . '\', \'' . $location . '\', \'' . $unique_id . '\'); return false;">' . $poll_options['vote_button_label'] . '</button>', $template );
				}
				else {
					...
					if ( 'yes' == $poll_options['view_back_to_vote_link'] ){
						...
						$this->vote = false;
						if ( !$this->is_voted() ){
							$template = str_ireplace( '%POLL-BACK-TO-VOTE-LINK%', '<a href="javascript:void(0)" class="shivs_poll_back_to_vote_link" id="shivs_poll_back_to_vote_link' . $poll_id . $unique_id . '" onClick="shivs_poll_back_to_vote(\'' . $poll_id . '\', \'' . $location . '\', \'' . $unique_id . '\')">' . $poll_options['view_back_to_vote_link_label'] . '</a>', $template );
						}
						$this->vote = $vote;
					}
				}
		...
		if ( '9999-12-31 23:59:59' == $poll_details['end_date'] ){
			$template = str_ireplace( '%POLL-END-DATE%', __( 'Never Expire', 'shivs_poll' ), $template );
		}
		else {
		...
		if ( 'yes' == $poll_options['view_results_link'] ){
			$template = str_ireplace( '%POLL-VIEW-RESULT-LINK%', '<a href="javascript:void(0)" class="shivs_poll_result_link" id="shivs_poll_result_link' . $poll_id . $unique_id . '" onClick="shivs_poll_view_results(\'' . $poll_id . '\', \'' . $location . '\', \'' . $unique_id . '\')">' . $poll_options['view_results_link_label'] . '</a>', $template );
		}

		if ( 'yes' == $poll_options['view_poll_archive_link'] ){
			$template = str_ireplace( '%POLL-VIEW-ARCHIVE-LINK%', '<a href="' . $poll_options['poll_archive_url'] . '" class="shivs_poll_archive_link" id="shivs_poll_archive_link_' . $poll_id . $unique_id . '" >' . $poll_options['view_poll_archive_link_label'] . '</a>', $template );
		}
		...
		if ( strpos( $template, "%POLL-SUCCESS-MSG%" ) != false ){
			$msgDivS  = true;
			$template = str_ireplace( '%POLL-SUCCESS-MSG%', '<div id="shivs-poll-container-success-' . $poll_id . $unique_id . '" class="shivs-poll-container-success"></div>', $template );
		}
		if ( strpos( $template, "%POLL-ERROR-MSG%" ) != false ){
			$msgDivE  = true;
			$template = str_ireplace( '%POLL-ERROR-MSG%', '<div id="shivs-poll-container-error-' . $poll_id . $unique_id . '" class="shivs-poll-container-error"></div>', $template );
		}
		...
		$template .= '<div id="shivs-poll-container-' . $poll_id . $unique_id . '" class="shivs-poll-container">';
		if ( !$msgDivS ){
			$template .= '<div id="shivs-poll-container-success-' . $poll_id . $unique_id . '" class="shivs-poll-container-success"></div>';
		}
		if ( !$msgDivE ){
			$template .= '<div id="shivs-poll-container-error-' . $poll_id . $unique_id . '" class="shivs-poll-container-error"></div>';
		}

		$template .= '<form id="shivs-poll-form-' . $poll_id . $unique_id . '" class="shivs-poll-forms">' . $temp . '<input type="hidden" id="shivs-poll-tr-id-' . $poll_id . $unique_id . '" name="shivs_poll_tr_id" value="' . $tr_id . '"/>' . wp_nonce_field( 'shivs_poll-' . $poll_id . $unique_id . '-user-actions', 'shivs-poll-nonce-' . $poll_id . $unique_id, false, false ) . '</form></div>';
		return $template;
	}
	...
	public function answer_result_replace_callback( $m ) {
		...
		if ( $this->is_view_poll_results() ){
			...
			if ( count( $answers ) > 0 ){
				foreach ( $answers as $answer ) {
					if ( function_exists( 'icl_translate' ) ){
						$answer['answer'] = icl_translate( 'shivs_poll', $answer['id'] . '_answer', $answer['answer'] );
					}
					$poll_options   = $this->poll_options;
					$answer_options = get_shivs_poll_answer_meta( $answer['id'], 'options', true );
					if ( $answer_options ){
					...
					if ( 'yes' == $poll_options['poll_answer_html_tags'] ){
						$temp_string = str_ireplace( '%POLL-ANSWER-LABEL%', shivs_poll_kses( stripslashes( $answer['answer'] ) ), $temp_string );
					}
					else {
						$temp_string = str_ireplace( '%POLL-ANSWER-LABEL%', shivs_poll_kses( esc_html( stripslashes( $answer['answer'] ) ) ), $temp_string );
					}
	...
	public function customfield_replace_callback( $m ) {
		...
		if ( !$is_voted ){
			...
			if ( count( $custom_fields ) > 0 ){
				foreach ( $custom_fields as $custom_field ) {
					if ( function_exists( 'icl_translate' ) ){
						$custom_field['custom_field'] = icl_translate( 'shivs_poll', $custom_field['id'] . '_custom_field', $custom_field['custom_field'] );
					}
					if ( 'yes' == $poll_options['poll_custom_field_html_tags'] ){
						$temp_string = str_ireplace( '%POLL-CUSTOM-FIELD-LABEL%', '<label for="shivs-poll-customfield-' . $custom_field['id'] . '">' . strip_tags( stripslashes( $custom_field['custom_field'] ) ) . '</label>', $m[5] );
					}
					else {
						$temp_string = str_ireplace( '%POLL-CUSTOM-FIELD-LABEL%', '<label for="shivs-poll-customfield-' . $custom_field['id'] . '">' . strip_tags( esc_html( stripslashes( $custom_field['custom_field'] ) ) ) . '</label>', $m[5] );
					}
					$temp_string = str_ireplace( '%POLL-CUSTOM-FIELD-TEXT-INPUT%', '<input type="text" value="" name="shivs_poll_customfield[' . $custom_field['id'] . ']" id="shivs-poll-customfield-' . $custom_field['id'] . '" />', $temp_string );
					$return_string .= $temp_string;
				}
			}
		}
		...
		if ( !$is_voted ){
			...
			if ( isset( $poll_options['allow_other_answers'] ) ){
				if ( 'yes' == $poll_options['allow_other_answers'] ){
					...
					if ( !$other_answer ){
						$answer          = array( 'id' => NULL, 'poll_id' => $poll_id, 'answer' => isset( $poll_options['other_answers_label'] ) ? shivs_poll_kses( $poll_options['other_answers_label'] ) : __( 'Other', 'shivs_poll' ), 'votes' => 0, 'status' => 'active', 'type' => 'other' );
					...
					if ( function_exists( 'icl_translate' ) ){
						$other_answer_label = icl_translate( 'shivs_poll', $poll_id . '_other_answer_label', shivs_poll_kses( stripslashes( $other_answer[0]['answer'] ) ) );
					}
					else {
						$other_answer_label = shivs_poll_kses( stripslashes( $other_answer[0]['answer'] ) );
					}

					if ( $multiple_answers ){
						if ( isset( $poll_options['is_default_answer'] ) && 'yes' == $poll_options['is_default_answer'] ){
							$temp_string = str_ireplace( '%POLL-OTHER-ANSWER-CHECK-INPUT%', '<input checked="checked" type="checkbox" value="' . $other_answer[0]['id'] . '" name="shivs_poll_answer[' . $other_answer[0]['id'] . ']" id="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $other_answer[0]['id'] . $unique_id . '" />', $m[5] );
						}
						else {
							$temp_string = str_ireplace( '%POLL-OTHER-ANSWER-CHECK-INPUT%', '<input type="checkbox" value="' . $other_answer[0]['id'] . '" name="shivs_poll_answer[' . $other_answer[0]['id'] . ']" id="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $other_answer[0]['id'] . $unique_id . '" />', $m[5] );
						}
					}
					else {
						if ( isset( $poll_options['is_default_answer'] ) && 'yes' == $poll_options['is_default_answer'] ){
							$temp_string = str_ireplace( '%POLL-OTHER-ANSWER-CHECK-INPUT%', '<input checked="checked" type="radio" value="' . $other_answer[0]['id'] . '" name="shivs_poll_answer" id="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $other_answer[0]['id'] . $unique_id . '" />', $m[5] );
						}
						else {
							$temp_string = str_ireplace( '%POLL-OTHER-ANSWER-CHECK-INPUT%', '<input type="radio" value="' . $other_answer[0]['id'] . '" name="shivs_poll_answer" id="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $other_answer[0]['id'] . $unique_id . '" />', $m[5] );
						}
					}
					if ( 'yes' == $poll_options['poll_answer_html_tags'] ){
						$temp_string = str_ireplace( '%POLL-OTHER-ANSWER-LABEL%', '<label for="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $other_answer[0]['id'] . $unique_id . '">' . stripslashes( $other_answer_label ) . '</label>', $temp_string );
					}
					else {
						$temp_string = str_ireplace( '%POLL-OTHER-ANSWER-LABEL%', '<label for="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $other_answer[0]['id'] . $unique_id . '">' . esc_html( stripslashes( $other_answer_label ) ) . '</label>', $temp_string );
					}
					$temp_string = str_ireplace( '%POLL-OTHER-ANSWER-TEXT-INPUT%', '<label><input onclick="document.getElementById(\'shivs-poll-answer-' . $poll_id . $unique_id . '-' . $other_answer[0]['id'] . $unique_id . '\').checked=true;" type="text" value="" name="shivs_poll_other_answer" id="shivs-poll-other-answer-' . $poll_id . $unique_id . '-' . $other_answer[0]['id'] . '" /></label>', $temp_string );
					if ( $this->is_view_poll_results() ){
				...
				foreach ( $answers as $answer ) {
					...
					if ( $multiple_answers ){
						if ( isset( $answer_options['is_default_answer'] ) && 'yes' == $answer_options['is_default_answer'] ){
							$temp_string = str_ireplace( '%POLL-ANSWER-CHECK-INPUT%', '<input type="checkbox" checked="checked" value="' . $answer['id'] . '" name="shivs_poll_answer[' . $answer['id'] . ']" id="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $answer['id'] . '" />', $m[5] );
						}
						else {
							$temp_string = str_ireplace( '%POLL-ANSWER-CHECK-INPUT%', '<input type="checkbox" value="' . $answer['id'] . '" name="shivs_poll_answer[' . $answer['id'] . ']" id="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $answer['id'] . '" />', $m[5] );
						}
					}
					else {
						if ( isset( $answer_options['is_default_answer'] ) && 'yes' == $answer_options['is_default_answer'] ){
							$temp_string = str_ireplace( '%POLL-ANSWER-CHECK-INPUT%', '<input type="radio" checked="checked" value="' . $answer['id'] . '" name="shivs_poll_answer" id="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $answer['id'] . '" />', $m[5] );
						}
						else {
							$temp_string = str_ireplace( '%POLL-ANSWER-CHECK-INPUT%', '<input type="radio" value="' . $answer['id'] . '" name="shivs_poll_answer" id="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $answer['id'] . '" />', $m[5] );
						}
					}
					if ( 'yes' == $poll_options['poll_answer_html_tags'] ){
						$temp_string = str_ireplace( '%POLL-ANSWER-LABEL%', '<label for="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $answer['id'] . '">' . shivs_poll_kses( stripslashes( $answer['answer'] ) ) . '</label>', $temp_string );
					}
					else {
						$temp_string = str_ireplace( '%POLL-ANSWER-LABEL%', '<label for="shivs-poll-answer-' . $poll_id . $unique_id . '-' . $answer['id'] . '">' . shivs_poll_kses( stripslashes( $answer['answer'] ) ) . '</label>', $temp_string );
					}
	...
	public function captcha_replace_callback( $m ) {
		...
		if ( 'yes' == $poll_options['use_captcha'] ){
			$sid         = md5( uniqid() );
			$temp_string = str_ireplace( '%CAPTCHA-IMAGE%', '<img class="shivs_poll_captcha_image" id="shivs_poll_captcha_image_' . $poll_id . $unique_id . '" src="' . admin_url( 'admin-ajax.php', ( is_ssl() ? 'https' : 'http' ) ) . '?action=shivs_poll_show_captcha&poll_id=' . $poll_id . '&sid=' . $sid . '&unique_id=' . $unique_id . '" />', $m[5] );
			$temp_string = str_ireplace( '%CAPTCHA-INPUT%', '<input type="text" value="" name="shivs_poll_captcha_input[' . $poll_id . ']" id="shivs-poll-captcha-input-' . $poll_id . $unique_id . '" />', $temp_string );
			$temp_string = str_ireplace( '%RELOAD-CAPTCHA-IMAGE%', '<a href="javascript:void(0)"><img src="' . SHIVS_POLL_URL . '/images/captcha_reload.png' . '" alt="' . __( 'Reload', 'shivs_poll' ) . '" onClick="shivs_poll_reloadCaptcha(' . "'" . $poll_id . "', '" . $unique_id . "'" . ')" /></a>', $temp_string );
			$temp_string = str_ireplace( '%CAPTCHA-LABEL%', __( 'Enter the code', 'shivs_poll' ), $temp_string );
			$temp_string = str_ireplace( '%CAPTCHA-PLAY%', '<object type="application/x-shockwave-flash" data="' . SHIVS_POLL_URL . '/captcha/securimage_play.swf?bgcol=#ffffff&amp;icon_file=' . SHIVS_POLL_URL . '/images/captcha-audio.gif&amp;audio_file=' . urlencode( admin_url( 'admin-ajax.php', ( is_ssl() ? 'https' : 'http' ) ) . '?action=shivs_poll_play_captcha&poll_id=' . $poll_id . '&unique_id=' . $unique_id ) . '" height="30" width="30">
				<param name="movie" value="' . SHIVS_POLL_URL . '/captcha/securimage_play.swf?bgcol=#ffffff&amp;icon_file=' . SHIVS_POLL_URL . '/images/captcha-audio.gif&amp;audio_file=' . urlencode( admin_url( 'admin-ajax.php', ( is_ssl() ? 'https' : 'http' ) ) . '?action=shivs_poll_play_captcha&poll_id=' . $poll_id . '&unique_id=' . $unique_id ) . '" />
				</object>', $temp_string );
		}
		$return_string .= $temp_string;
	...
	public static function display_poll_result_bar( $poll_id = 0, $answer_id = 0, $procent = 0, $poll_options = array(), $unique_id = '' ) {
		$result_bar = '';
		$result_bar = ' <div class="shivs-poll-results-bar-' . $poll_id . $unique_id . '" ';
		...
		if ( floatval( $procent ) > 0 ){
			...
			$result_bar .= '" ' . 'id="shivs-poll-result-bar-div-' . $answer_id . '" ' . 'class="shivs-poll-result-bar-div-' . $poll_id . $unique_id . '"' . '>' . '</div>';
		}
		$result_bar .= '</div>';
		return $result_bar;
	}
	...
	private function is_voted_cookie() {
		if ( isset( $_COOKIE['shivs_poll_voted_' . $this->poll['id']] ) ){
			return true;
		}
		return false;
	}
	...
	private function set_vote_cookie( $answer_ids = '0', $vote_type = 'default', $facebook_user_details = NULL ) {
		...
		setcookie( 'shivs_poll_voted_' . $this->poll['id'], $answer_ids, $expire_cookie, COOKIEPATH, COOKIE_DOMAIN, false );
		setcookie( 'shivs_poll_vote_type_' . $this->poll['id'], $vote_type, $expire_cookie, COOKIEPATH, COOKIE_DOMAIN, false );
		if ( 'facebook' == $vote_type ){
			setcookie( 'shivs_poll_vote_facebook_user_' . $this->poll['id'], $facebook_user_details['id'], $expire_cookie, COOKIEPATH, COOKIE_DOMAIN, false );
		}
	}
	...
	private function is_voted_username( $vote_type = 'default', $facebook_user_details = NULL, $from_register = NULL ) {
		...
		if ( !$from_register ){
			$vote_type                   = in_array( $_COOKIE['shivs_poll_vote_type_' . $this->poll['id']], $this->vote_types ) ? $_COOKIE['shivs_poll_vote_type_' . $this->poll['id']] : 'default';
			$facebook_user_details['id'] = $_COOKIE['shivs_poll_vote_facebook_user_' . $this->poll['id']];
		}
		...
		if ( 'facebook' == $vote_type ){
			$user_id = $wpdb->get_var( $wpdb->prepare( "
					SELECT id
					FROM " . $wpdb->shivs_poll_facebook_users . "
					WHERE
					fb_id = %d
